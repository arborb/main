...
public class WCService {
  ...
  public String reloadSqlMap() {
    ...
    SqlMapClientFactoryBean factory = (SqlMapClientFactoryBean)appContext.getBean(SqlMapClientFactoryBean.class);
  ...
  public Map getEncryptString(Map<String, Object> params) throws Exception {
    ...
    while (encryptKeys.hasNext()) {
      ...
      String encryptVal = commonEncryptor.encrypt((String)params.get(encryptKey));
  ...
  public Map getEncryptHash(Map<String, Object> params) throws Exception {
    ...
    while (encryptKeys.hasNext()) {
      String encryptKey = (String)encryptKeys.next();
      String encryptVal = commonEncryptor.encryptHASH((String)params.get(encryptKey));
      result.put(encryptKey, encryptVal);
    }
    return result;
  }
  ...
  public Map callFn(Map<String, Object> params) throws Exception {
    ...
    if ("GENKEY".equalsIgnoreCase(CMD)) {
      result.put(PK_O_RESULT_DATA, Boolean.toString(commonEncryptor.generateKeyFiles(callParams[0], callParams[1])));
    } else if ("ENCRSA0000".equalsIgnoreCase(CMD)) {
      result.put(PK_O_RESULT_DATA, commonEncryptor.encryptRSA(callParams[0]));
    } else if ("DECRSA0000".equalsIgnoreCase(CMD)) {
      result.put(PK_O_RESULT_DATA, commonEncryptor.decryptRSA(callParams[0]));
    } else if ("ENCRSA0010".equalsIgnoreCase(CMD)) {
      result.put(PK_O_RESULT_DATA, commonEncryptor.encryptRSA(callParams[0], "nexos/config/publicKey_0010.xml"));
    } else if ("DECRSA0010".equalsIgnoreCase(CMD)) {
      result.put(PK_O_RESULT_DATA, commonEncryptor.decryptRSA(callParams[0], "nexos/config/privateKey_0010.xml"));
    } else {
