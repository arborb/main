...
/**
 * Class: LC02040EService<br>
 * Description: 세트상품관리(LC02040E) 서비스를 담당하는 Class(트랜잭션처리 담당)<br>
...
public class LC02040EService {
  ...
  public String callLCProcessing(String processDiv, Map<String, Object> params) throws Exception {
    ...
    // TransactionStatus ts = commonDAO.beginTrans();
    ...
    try {
      ...
      if (!Consts.OK.equals(oMsg_E) || !Consts.OK.equals(oMsg_D)) {
        ...
        // 상품변환 확정, 확정 취소시 변동가용재고 송신
        if (LC_FW_ETC_OUTCONFIRM.equals(PROCEDURE_ID_D) || LC_BW_ETC_OUTCONFIRM.equals(PROCEDURE_ID_D)
          || LC_FW_ETC_INCONFIRM.equals(PROCEDURE_ID_E) || LC_BW_ETC_INCONFIRM.equals(PROCEDURE_ID_E)) {
          // 주문상태 송신 및 가용재고 송신 호출
          oMsg_D = edCommonService.realtimeSendProcessing();
          // 오류면 Rollback
          if (!Consts.OK.equals(oMsg_D)) {
            throw new RuntimeException(oMsg_D);
          }
        }

        transactionManager.commit(ts);
        // commonDAO.commitTrans(ts);
