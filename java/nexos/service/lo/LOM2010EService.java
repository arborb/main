...
/**
 * Class: LOM2010EService<br>
 * Description: 출고작업(LOM2010E) 서비스를 담당하는 Class(트랜잭션처리 담당)<br>
 ...
 */
@Service("LOM2010E")
public class LOM2010EService {
 
  // private final Logger logger = LoggerFactory.getLogger(LOM2010EService.class);

  /**
   * DAO 주입처리하기
   */
  @Resource
  private LOM2010EDAO                dao;
  ...
  /**
   * LO_FW_DIRECTIONS_INVNO_PROC 호출
   *
   * @param params
   * @return
   * @throws Exception
  ...
  public String callInvProc(Map<String, Object> params) throws Exception {

    final String INVNO_PROCEDURE_ID = "LO_FW_DIRECTIONS_INVNO_PROC";
    ...
    for (int i = 0; i < dsCnt; i++) {
      ...
      try {
        ...
        mapResult = callSP(INVNO_PROCEDURE_ID, callParams);
   ...
   */
  @SuppressWarnings("unchecked")
  public String callInvProcLine(Map<String, Object> params) throws Exception {

    final String FW_PROCEDURE_ID = "LO_FW_DIRECTIONS_INVNO_LINE";
      ...
      try {
        ...
        if (Consts.PROCESS_ENTRY_T1.equals(process_Cd) && Consts.DIRECTION_FW.equals(direction)) {
          ...
          if (Consts.PROCESS_DIRECTIONS.equals(process_Cd) && Consts.DIRECTION_FW.equals(direction)) {
            ...
            /*
             * if (ship_type == null) {
             * ship_type = (String)params.get("P_SHIP_TYPE");
   ...
   */
  @SuppressWarnings("unchecked")
  public String callProcShipType(Map<String, Object> params) throws Exception {

    final String LO_FW_DIRECTIONS_SHIPTYPE_PROCEDURE_ID = "LO_FW_DIRECTIONS_SHIPTYPE_PROC";
    ...
    for (int i = 0; i < dsCnt; i++) {
      ...
      try {
        ...
        mapResult = callSP(LO_FW_DIRECTIONS_SHIPTYPE_PROCEDURE_ID, callParams);
  ...
  public String callWbProc(String queryId, Map<String, Object> params) throws Exception {
    ...
    try {
      ...
      // 오류면 Rollback
      if (!Consts.YES.equals(oMsg)) {
        transactionManager.rollback(ts);
        sbResult.append(oMsg);
        sbResult.append(Consts.CRLF);
      } else if (Consts.YES.equals(oMsg)) {
        transactionManager.commit(ts);
        sbResult.append(Consts.YES);
      }
    } catch (Exception e) {
      // SP 내에서 오류가 아니면 Exit
      transactionManager.rollback(ts);
...
/*
 ...
 public Map callUpdateD(Map<String, Object> params) throws Exception {
   ...
   TransactionStatus ts = transactionManager.getTransaction(new DefaultTransactionDefinition());
   try {
     result = (Map)dao.callUpdateD(params);
     String oMsg = (String)result.get(Consts.PK_O_MSG);
     // 오류면 Rollback
     if (!Consts.OK.equals(oMsg)) {
       transactionManager.rollback(ts);
