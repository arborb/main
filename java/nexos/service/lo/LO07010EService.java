...
/**
 * Class: LO07010EService<br>
 * Description: 출고스캔검수(LO07010E) 서비스를 담당하는 Class(트랜잭션처리 담당)<br>
 ...
 */
@Service("LO07010E")
public class LO07010EService {

  /**
   * DAO 주입처리하기
   */
  @Resource
  private LO07010EDAO                dao;
  ...
  public Map callScanBoxMerge(Map<String, Object> params) throws Exception {
    ...
    TransactionStatus ts = transactionManager.getTransaction(new DefaultTransactionDefinition());
    try {
      result = (Map)dao.callScanBoxMerge(params);
  ...
  public Map callFWScanConfirm(Map<String, Object> params) throws Exception {
    ...
    TransactionStatus ts = transactionManager.getTransaction(new DefaultTransactionDefinition());
    try {
      result = (Map)dao.callFWScanConfirm(params);
      String oMsg = (String)result.get(Consts.PK_O_MSG);
      // 오류면 Rollback
      if (!Consts.OK.equals(oMsg)) {
        throw new RuntimeException(oMsg);
  ...
  public Map callBWScanConfirm(Map<String, Object> params) throws Exception {
    ...
    TransactionStatus ts = transactionManager.getTransaction(new DefaultTransactionDefinition());
    try {
      result = (Map)dao.callBWScanConfirm(params);
      String oMsg = (String)result.get(Consts.PK_O_MSG);
      // 오류면 Rollback
      if (!Consts.OK.equals(oMsg)) {
        throw new RuntimeException(oMsg);
