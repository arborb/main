...
//  $("#btnQDelivery_Cd").click(showDeliveryPopup);
  $("#btnRiorderCall").click(callRiorderSp); // 업체환불요청 버튼 클릭
...
//  var DELIVERY_CD = $NC.getValue("#edtQDelivery_Cd", true);
  ...
  if ($NC.G_VAR.activeView.PROCESS_CD === "B") {
    ...
    // 데이터 조회
    $NC.serviceCall("/RIM2010E/getDataSet.do", $NC.getGridParams(G_GRDMASTERB), onGetMasterB);
    return;
  }
  ...
  if ($NC.G_VAR.activeView.PROCESS_CD === "C") {
    ...
    // 데이터 조회
    $NC.serviceCall("/RIM2010E/getDataSet.do", $NC.getGridParams(G_GRDMASTERC), onGetMasterC);
  ...
  if ($NC.G_VAR.activeView.PROCESS_CD === "D") {
    ...
    // 데이터 조회
    $NC.serviceCall("/RIM2010E/getDataSet.do", $NC.getGridParams(G_GRDMASTERD), onGetMasterD);
    return;
  }
  ...
  if ($NC.G_VAR.activeView.PROCESS_CD === "E") {
    ...
    // 데이터 조회
    $NC.serviceCall("/RIM2010E/getDataSet.do", $NC.getGridParams(G_GRDMASTERE), onGetMasterE);
    return;
...
function _Save() {
  ...
  if ($NC.G_VAR.activeView.PROCESS_CD === "C") {
    ...
    if ($NC.G_VAR.policyVal.RI230 == "1") {
      ...
      if (saveDS.length > 0) {
        $NC.serviceCall("/RIM2010E/saveDirections.do", {
      ...
      if (saveDS.length > 0) {
        $NC.serviceCall("/RIM2010E/saveDirectionsLocId.do", {
          P_DS_SUB: $NC.toJson(saveDS),
          P_USER_ID: $NC.G_USERINFO.USER_ID
        }, onSaveC);
      }
