function onProcessPreB() {
  ...
  for (var row = 0; row < rowCount; row++) {
    ...
    if (rowData.CHECK_YN == "Y") {
      if (rowData.INOUT_SUB_CD === "E8") {
        alert("반입작업화면에서 미배송입고 전표에 대하여 취소 처리할 수 없습니다.\n미배송입고전표를 취소하시려면 배송완료화면에서 해당 전표에 대하여 취소하십시오.");
        return;
      }
      chkCnt++;
      // 반입등록 상태인 전표만 대상
...
function onProcessNxtB() {
  ...
  $NC.serviceCall("/RI02010E/callRIEntryProcessing.do", {
    P_DS_MASTER: $NC.getParams(processDS),
    P_PROCESS_CD: "B1",
    P_DIRECTION: "FW",
    P_PROCESS_STATE_BW: $NC.G_VAR.stateFWBW[$NC.G_VAR.activeView.PROCESS_CD].CANCEL,
...
function grdMasterBOnAfterScroll(e, args) {
  ...
  if (rowData.CRUD !== "C" && rowData.CRUD !== "N") {
    ...
    // 데이터 조회
    $NC.serviceCall("/RI02010E/getDataSet.do", $NC.getGridParams(G_GRDDETAILB), onGetDetailB);
  }
...
function grdMasterBOnDblClick(e, args) {
  ...
  if (masterRowData.INOUT_SUB_CD === "E8") {
    alert("반입작업화면에서 미배송입고 전표에 대하여 수정 처리할 수 없습니다.\n미배송입고전표를 취소하시려면 배송완료화면에서 해당 전표에 대하여 취소하십시오.");
    return;
  }
