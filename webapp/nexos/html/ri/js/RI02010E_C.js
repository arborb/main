function onProcessPreC() {
  ...
  var result = confirm("반입지시 취소 처리하시겠습니까?");
  if (!result) {
    return;
  }
  ...
  if (chkCnt == 0) {
    alert("반입지시 취소 처리할 데이터를 선택하십시오.");
    return;
  }
  if (processDS.length == 0) {
    alert("선택한 데이터 중 반입지시 취소 처리 가능한 데이터가 없습니다.");
    return;
  }

  $NC.serviceCall("/RI02010E/callRIProcessing.do", {
...
function onProcessNxtC() {
  ...
  var result = confirm("반입지시 처리하시겠습니까?");
  if (!result) {
    return;
  }
  ...
  if ($NC.isGridModified(G_GRDSUBC)) {
    alert("데이터가 수정되었습니다. 저장 후 반입지시 처리하십시오.");
    return;
  }
  ...
  if (chkCnt == 0) {
    alert("반입지시 처리할 데이터를 선택하십시오.");
    return;
  }
  if (processDS.length == 0) {
    alert("선택한 데이터 중 반입지시 처리 가능한 데이터가 없습니다.");
    return;
  }
  ...
  if (!$NC.isNull(resultData)) {
    if (resultData.O_MSG === "OK") {
      ...
      $NC.serviceCall("/RI02010E/getDataSet.do", {
        P_QUERY_ID: "RI02010E.RS_T2_SUB1",
        P_QUERY_PARAMS: $NC.getParams({
          P_CENTER_CD: rowData.CENTER_CD,
          P_BU_CD: rowData.BU_CD,
          P_INBOUND_DATE: rowData.INBOUND_DATE,
...
function onNewDirectionsC() {
  ...
  var newRowData = {
    ...
    DELIVERY_CD: masterRowData.DELIVERY_CD,
    ...
    BRAND_NM: detailRowData.BRAND_NM,
...
function grdMasterCOnAfterScroll(e, args) {
  ...
  // 데이터 조회
  $NC.serviceCall("/RI02010E/getDataSet.do", $NC.getGridParams(G_GRDDETAILC), onGetDetailC);
  ...
  // 데이터 조회
  $NC.serviceCall("/RI02010E/getDataSet.do", $NC.getGridParams(G_GRDLOCATIONIDC), onGetLocationIdC);
  ...
  // 데이터 조회
  $NC.serviceCall("/RI02010E/getDataSet.do", $NC.getGridParams(G_GRDSUBC), onGetSubC);
...
function grdDetailCOnGetColumns(policyRI420) {
...
function grdDetailCInitialize() {
  ...
  // Grid 더블클릭 이벤트
  G_GRDDETAILC.view.onDblClick.subscribe(function(e, args) {
...
function grdSubCOnCellChange(e, args) {
  ...
  if (args.cell === G_GRDSUBC.view.getColumnIndex("ENTRY_QTY")) {
...
function grdSubCOnBeforePost(row) {
  ...
  if (rowData.CRUD != "R") {
    ...
    // 디테일에 해당 LINE_NO를 검색하여 등록 수량 가져오기
    var rowCount = G_GRDDETAILC.data.getLength();
    var detailRowData;
    for (var i = 0; i < rowCount; i++) {
      detailRowData = G_GRDDETAILC.data.getItem(i);
      if (rowData.LINE_NO === detailRowData.LINE_NO) {
    ...
    if (detailRowData.LINE_NO !== rowData.LINE_NO) {
      alert("지시수량을 반영하지 못했습니다. 다시 작업하십시오.");
      rowData.ENTRY_QTY = 0;
      G_GRDSUBC.data.updateItem(rowData.id, rowData);
      $NC.setGridSelectRow(G_GRDSUBC, {
        selectRow: row,
...
function grdSubCOnBeforeEditCell(e, args) {
  ...
  if (args.column.field === "ENTRY_QTY" && $NC.G_VAR.policyVal.RI230 !== "1") {
    return true;
  }
  return false;
}
...
function grdLocationIdCOnGetColumns() {
