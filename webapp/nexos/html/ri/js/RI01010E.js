...
function _New() {
  ...
  if ($NC.G_VAR.policyVal.RI110 !== "Y") {
    alert("반입예정정보 신규등록/수정이 불가능한 브랜드입니다.");
    return;
  }
...
function _Save() {
  ...
  if (saveDS.length > 0) {
    $NC.serviceCall("/RI01010E/delete.do", {
...
function grdMasterOnAfterScroll(e, args) {
  ...
  if (rowData.CRUD !== "C" && rowData.CRUD !== "N") {
    ...
    // 데이터 조회
    $NC.serviceCall("/RI01010E/getDataSet.do", $NC.getGridParams(G_GRDDETAIL), onGetDetail);
  }
...
function getInboundState(params, onSuccess) {
  ...
  // 데이터 조회
  $NC.serviceCall("/RI01010E/callSP.do", {
