...
function _Initialize() {
  ...
  // ///////////////////////////////////////////////////////////////
  // 정산항목
  $NC.serviceCallAndWait("/LF02020E/getDataSet.do", {
    P_QUERY_ID: "LF02020E.RS_SUB1",
    P_QUERY_PARAMS: ""
  }, function(ajaxData) {
    $NC.G_VAR.FEE_HEAD_CD = $NC.toArray(ajaxData);
  });

  // 정산세부항목
  $NC.serviceCall("/LF02020E/getDataSet.do", {
    P_QUERY_ID: "LF02020E.RS_SUB2",
    P_QUERY_PARAMS: ""
  }, function(ajaxData) {
  ...
  $NC.serviceCallAndWait("/WC/getDataSet.do", {
    P_QUERY_ID: "WC.POP_CMCODE",
    P_QUERY_PARAMS: $NC.getParams({
      P_CODE_GRP: "CONSIGN_UNIT_DIV",
      P_CODE_CD: "%",
      P_SUB_CD1: "",
      P_SUB_CD2: ""
    })
  }, function(ajaxData) {
    $NC.G_VAR.CONSIGN_UNIT_DIV = $NC.toArray(ajaxData);
  });
...
function _Inquiry() {
  ...
  // 데이터 조회
  $NC.serviceCall("/LF02020E/getDataSet.do", $NC.getGridParams(G_GRDMASTER), onGetMaster);
...
function _New() {
  ...
  if (G_GRDMASTER.focused) {
    ...
    var newRowData = {
      ...
      CONSIGN_UNIT_DIV: groupData.CONSIGN_UNIT_DIV,
      CONSIGN_UNIT_DIV_F: $NC.getGridComboName(G_GRDDETAIL1, {
        colFullNameField: "CONSIGN_UNIT_DIV_F",
        searchVal: groupData.CONSIGN_UNIT_DIV,
        dataCodeField: "CODE_CD",
        dataFullNameField: "CODE_CD_F"
      }),
      CONSIGN_PRICE: "0",
    ...
    var newRowData = {
      ...
      PERIOD_DIV: groupData.PERIOD_DIV,
      CONSIGN_UNIT_DIV: groupData.CONSIGN_UNIT_DIV,
      CONSIGN_UNIT_DIV_F: $NC.getGridComboName(G_GRDDETAIL2, {
        colFullNameField: "CONSIGN_UNIT_DIV_F",
        searchVal: groupData.CONSIGN_UNIT_DIV,
        dataCodeField: "CODE_CD",
        dataFullNameField: "CODE_CD_F"
    ...
    var newRowData = {
      ...
      PERIOD_DIV: groupData.PERIOD_DIV,
      CONSIGN_UNIT_DIV: groupData.CONSIGN_UNIT_DIV,
      CONSIGN_UNIT_DIV_F: $NC.getGridComboName(G_GRDDETAIL3, {
        colFullNameField: "CONSIGN_UNIT_DIV_F",
        searchVal: groupData.CONSIGN_UNIT_DIV,
        dataCodeField: "CODE_CD",
        dataFullNameField: "CODE_CD_F"
...
function _Save() {
  ...
  for (var row = 0; row < rowCount; row++) {
    ...
    if (rowData.CRUD !== "R") {
      var saveData = {
        ...
        P_CONSIGN_UNIT_DIV: rowData.CONSIGN_UNIT_DIV,
        P_CONSIGN_PRICE: rowData.CONSIGN_PRICE,
  ...
  if (saveMasterDS.length > 0 || saveDetailDS.length > 0) {
    $NC.serviceCall("/LF02020E/save.do", {
...
function grdMasterOnGetColumns() {
  ...
  $NC.setGridColumn(columns, {
    id: "CONSIGN_UNIT_DIV_F",
    field: "CONSIGN_UNIT_DIV_F",
    name: "단위구분",
    minWidth: 120,
    editor: Slick.Editors.ComboBox,
    editorOptions: {
      codeField: "CONSIGN_UNIT_DIV",
      dataCodeField: "CODE_CD",
      dataFullNameField: "CODE_CD_F",
      data: $NC.G_VAR.CONSIGN_UNIT_DIV,
      isKeyField: true
    }
  });
  $NC.setGridColumn(columns, {
...
function grdMasterOnAfterScroll(e, args) {
  ...
  if (rowData.CRUD == "C" || rowData.CRUD == "N") {
    ...
    // 데이터 조회
    $NC.serviceCall("/LF02020E/getDataSet.do", $NC.getGridParams(G_GRDDETAIL1), onGetDetail1);
    ...
    // 데이터 조회
    $NC.serviceCall("/LF02020E/getDataSet.do", $NC.getGridParams(G_GRDDETAIL2), onGetDetail2);
    ...
    // 데이터 조회
    $NC.serviceCall("/LF02020E/getDataSet.do", $NC.getGridParams(G_GRDDETAIL3), onGetDetail3);
  }
...
function grdMasterOnBeforeEditCell(e, args) {
  ...
  if (args.column.field === "FEE_BASE_CD_F") {
    if ($NC.isNull(rowData.FEE_HEAD_CD)) {
      return false;
    }
  }
  if (args.column.field === "PERIOD_DIV_F") {
    if ($NC.isNull(rowData.FEE_BASE_CD)) {
      return false;
    }
  }
  if (args.column.field === "CONSIGN_UNIT_DIV_F") {
    if ($NC.isNull(rowData.FEE_BASE_CD)) {
      return false;
    }
  }
  ...
  if (args.column.field !== "FEE_HEAD_CD_F" && args.column.field !== "FEE_BASE_CD_F"
...
function grdMasterOnCellChange(e, args) {
  ...
  if (args.cell === G_GRDMASTER.view.getColumnIndex("FEE_HEAD_CD_F")) {
    rowData.FEE_BASE_CD = null;
    rowData.FEE_BASE_CD_F = null;

    var newArray = [ ];
    for (var row = 0; row < $NC.G_VAR.FEE_BASE_CD.length; row++) {
      if ($NC.G_VAR.FEE_BASE_CD[row].FEE_HEAD_CD === rowData.FEE_HEAD_CD) {
        newArray.push($NC.G_VAR.FEE_BASE_CD[row]);
      }
    }

    var cols = G_GRDMASTER.view.getColumns();
    cols[G_GRDMASTER.view.getColumnIndex("FEE_BASE_CD_F")].editorOptions = {
    ...
    if ($NC.isNull(rowData.FEE_BASE_CD)) {
      // 보관수수료일 경우 기본값
      if (rowData.FEE_HEAD_CD === "100") {
        rowData.FEE_BASE_CD = "010";
        rowData.FEE_BASE_CD_F = $NC.getGridComboName(G_GRDMASTER, {
          colFullNameField: "FEE_BASE_CD_F",
          searchVal: "010",
          dataCodeField: "FEE_BASE_CD",
          dataFullNameField: "FEE_BASE_CD_F"
        });

        rowData.PERIOD_DIV = "3";
        rowData.PERIOD_DIV_F = $NC.getGridComboName(G_GRDMASTER, {
          colFullNameField: "PERIOD_DIV_F",
          searchVal: "3",
          dataCodeField: "CODE_CD",
          dataFullNameField: "CODE_CD_F"
        });

        rowData.CONSIGN_UNIT_DIV = "13";
        rowData.CONSIGN_UNIT_DIV_F = $NC.getGridComboName(G_GRDMASTER, {
          colFullNameField: "CONSIGN_UNIT_DIV_F",
          searchVal: "13",
          dataCodeField: "CODE_CD",
          dataFullNameField: "CODE_CD_F"
...
function grdMasterOnNewRecord(args) {
  ...
  $NC.setFocusGrid(G_GRDMASTER, args.row, G_GRDMASTER.view.getColumnIndex("FEE_HEAD_CD_F"), true);
}
...
function grdMasterOnBeforePost(row) {
  ...
  if (rowData.CRUD == "N") {
    if ($NC.isNull(rowData.FEE_HEAD_CD) || $NC.isNull(rowData.FEE_BASE_CD) || $NC.isNull(rowData.CONTRACT_START_DATE)) {
      G_GRDMASTER.data.deleteItem(rowData.id);
      if (row > 0) {
  ...
  if (rowData.CRUD != "R") {
    if ($NC.isNull(rowData.FEE_HEAD_CD)) {
      ...
      $NC.setFocusGrid(G_GRDMASTER, G_GRDMASTER.lastRow, G_GRDMASTER.view.getColumnIndex("FEE_HEAD_CD_F"), true);
      return false;
    }
    if ($NC.isNull(rowData.FEE_BASE_CD)) {
    ...
    if ($NC.isNull(rowData.CONSIGN_UNIT_DIV)) {
      alert("단위구분을 선택하십시오.");
      $NC.setGridSelectRow(G_GRDMASTER, row);
      $NC.setFocusGrid(G_GRDMASTER, G_GRDMASTER.lastRow, G_GRDMASTER.view.getColumnIndex("CONSIGN_UIT_DIV_F"), true);
      return false;
    }
    if ($NC.isNull(rowData.CONSIGN_PRICE)) {
      alert("기준단가를 입력하십시오.");
      $NC.setGridSelectRow(G_GRDMASTER, row);
      $NC.setFocusGrid(G_GRDMASTER, G_GRDMASTER.lastRow, G_GRDMASTER.view.getColumnIndex("CONSIGN_PRICE"), true);
      return false;
    }
...
function grdDetail1OnBeforePost(row) {
  ...
  if (rowData.CRUD != "R") {
    ...
    if ($NC.isNull(rowData.CONSIGN_PRICE)) {
      alert("기준단가를 입력하십시오.");
      $NC.setGridSelectRow(G_GRDDETAIL1, row);
      $NC.setFocusGrid(G_GRDDETAIL1, G_GRDDETAIL1.lastRow, G_GRDDETAIL1.view.getColumnIndex("CONSIGN_PRICE"), true);
      return false;
    }
    if ($NC.isNull(rowData.CALC_QTY_DIV)) {
      alert("수량계산구분을 선택하십시오.");
...
function grdDetail2OnBeforePost(row) {
  ...
  if (rowData.CRUD != "R") {
    ...
    if ($NC.isNull(rowData.CONSIGN_PRICE)) {
      alert("기준단가를 입력하십시오.");
      $NC.setGridSelectRow(G_GRDDETAIL2, row);
      $NC.setFocusGrid(G_GRDDETAIL2, G_GRDDETAIL2.lastRow, G_GRDDETAIL2.view.getColumnIndex("CONSIGN_PRICE"), true);
      return false;
    }
    if ($NC.isNull(rowData.CALC_QTY_DIV)) {
      alert("수량계산구분을 선택하십시오.");
...
function grdDetail3OnBeforePost(row) {
  ...
  if (rowData.CRUD != "R") {
    ...
    if ($NC.isNull(rowData.CONSIGN_PRICE)) {
      alert("기준단가를 입력하십시오.");
      $NC.setGridSelectRow(G_GRDDETAIL3, row);
      $NC.setFocusGrid(G_GRDDETAIL3, G_GRDDETAIL3.lastRow, G_GRDDETAIL3.view.getColumnIndex("CONSIGN_PRICE"), true);
      return false;
    }
    if ($NC.isNull(rowData.CALC_QTY_DIV)) {
      alert("수량계산구분을 선택하십시오.");
...
function onGridItemPopup(resultInfo) {
  ...
  if (!$NC.isNull(resultInfo)) {
    ...
    focusCol = G_GRDDETAIL3.view.getColumnIndex("CONSIGN_UNIT_DIV_F");
  } else {
    rowData.ITEM_CD = "";
    rowData.ITEM_NM = "";
    rowData.ITEM_SPEC = "";
  ...
  if (G_GRDMASTER.data.getLength() > 0) {
    if ($NC.isNull(G_GRDMASTER.lastKeyVal)) {
      ...
      $NC.setGridSelectRow(G_GRDMASTER, {
        selectKey: new Array("FEE_HEAD_CD", "FEE_BASE_CD", "CONTRACT_START_DATE"),
