...
function _Inquiry() {
  ...
  // 데이터 조회
  $NC.serviceCall("/LF03040E/getDataSet.do", $NC.getGridParams(G_GRDMASTER), onGetMaster);
}
...
function _Save() {
  ...
  if (saveMasterDS.length > 0) {
    $NC.serviceCall("/LF03040E/save.do", {
...
function _Cancel() {
  ...
  var lastKeyVal = $NC.getGridLastKeyVal(G_GRDMASTER, {
    selectKey: ["FEE_HEAD_CD", "FEE_BASE_CD"],
    isCancel: true
  });
  _Inquiry();
  ...
  if (G_GRDMASTER.data.getLength() > 0) {
    if ($NC.isNull(G_GRDMASTER.lastKeyVal)) {
      ...
      $NC.setGridSelectRow(G_GRDMASTER, {
        selectKey: new Array("FEE_HEAD_CD", "FEE_BASE_CD"),
