...
function _Inquiry() {
  ...
  // 데이터 조회
  $NC.serviceCall("/LF02091E/getDataSet.do", $NC.getGridParams(G_GRDMASTER), onGetMaster);
}
...
function _Save() {
  ...
  if (saveMasterDS.length > 0) {
    $NC.serviceCall("/LF02091E/save.do", {
...
function grdMasterOnCellChange(e, args) {
  ...
  case "CARRIER_CD":
    ...
    if (O_RESULT_DATA.length <= 1) {
      onCarrier1Popup(O_RESULT_DATA[0]);
    } else {
      $NP.showCarrierPopup({
        queryParams: P_QUERY_PARAMS,
        queryData: O_RESULT_DATA
      }, onCarrier1Popup, onCarrier1Popup);
    }
    return;
  case "ITEM_CD_T":
...
function grdMasterOnBeforeEditCell(e, args) {
  ...
  if (args.column.field !== "CARRIER_CD" && args.column.field !== "FEE_HEAD_CD" && args.column.field !== "FEE_BASE_CD" && args.column.field !== "ITEM_CD_T") {
    return true;
  }
  ...
  if (rowData.CRUD !== "N" && rowData.CRUD !== "C") {
    ...
    if (args.column.field == "CARRIER_CD") {
      return false;
    }
    if (args.column.field == "ITEM_CD_T") {
      return false;
    }
 ...
 */
function onCarrier1Popup(resultInfo) {
...
function grdMasterOnPopup(e, args) {
  ...
  case "CARRIER_CD":
    $NP.showCarrierPopup({
      P_CARRIER_CD: rowData.CARRIER_CD,
      P_VIEW_DIV: "2"
    }, onCarrier1Popup, function() {
      $NC.setFocusGrid(G_GRDMASTER, G_GRDMASTER.lastRow, G_GRDMASTER.view.getColumnIndex("CARRIER_CD"), true, true);
    });
    break;
  case "FEE_HEAD_CD":
    $NP.showFee_Head_CdPopup({
...
function onCopyOwnBrand() {
  ...
  $NC.G_MAIN.showProgramSubPopup({
    PROGRAM_ID: "LF02092P",
    PROGRAM_NM: "운송사 단가 복사",
    url: "lf/LF02092P.html",
    width: 360,
    height: 160,
    userData: {
      P_CARRIER_CD: rowData.CARRIER_CD,
...
function onGridItemPopup(resultInfo) {
  ...
  if (!$NC.isNull(resultInfo)) {
    ...
    focusCol = G_GRDMASTER.view.getColumnIndex("PAY_UNIT_DIV_F");
  } else {
    rowData.ITEM_CD_T = "";
    rowData.ITEM_NM_T = "";
