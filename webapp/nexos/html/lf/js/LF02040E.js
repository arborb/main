...
function _Inquiry() {
  ...
  // 데이터 조회
  $NC.serviceCall("/LF02040E/getDataSet.do", $NC.getGridParams(G_GRDMASTER), onGetMaster);
}
...
function _Save() {
  ...
  for (var row = 0; row < rowCount; row++) {
    ...
    if (rowData.CRUD !== "R") {
      var saveData = {
        ...
        P_CONTRACT_PRICE: rowData.CONTRACT_PRICE,
  ...
  if (saveMasterDS.length > 0) {
    $NC.serviceCall("/LF02040E/save.do", {
...
function grdMasterOnGetColumns() {
  ...
  $NC.setGridColumn(columns, {
    ...
    editorOptions: $NC.getGridComboEditorOptions("/LF02040E/getDataSet.do", {
      P_QUERY_ID: "LF02040E.RS_SUB",
      P_QUERY_PARAMS: "{}"
    }, {
...
function grdMasterOnBeforePost(row) {
  ...
  if (rowData.CRUD != "R") {
    if ($NC.isNull(rowData.FEE_BASE_CD)) {
      alert("약정수수료구분을 선택하십시오.");
      $NC.setGridSelectRow(G_GRDMASTER, row);
      $NC.setFocusGrid(G_GRDMASTER, G_GRDMASTER.lastRow, G_GRDMASTER.view.getColumnIndex("FEE_BASE_CD"), true);
      return false;
    }
    if ($NC.isNull(rowData.CONTRACT_START_DATE)) {
    ...
    if ($NC.isNull(rowData.CONTRACT_PRICE)) {
      alert("약정단가를 입력하십시오.");
      $NC.setGridSelectRow(G_GRDMASTER, row);
      $NC.setFocusGrid(G_GRDMASTER, G_GRDMASTER.lastRow, G_GRDMASTER.view.getColumnIndex("CONTRACT_PRICE"), true);
      return false;
    }
    if (rowData.CONTRACT_PRICE <= 0) {
      alert("약정단가는 0보다 큰수를 입력하십시오.");
      $NC.setGridSelectRow(G_GRDMASTER, row);
      $NC.setFocusGrid(G_GRDMASTER, G_GRDMASTER.lastRow, G_GRDMASTER.view.getColumnIndex("CONTRACT_PRICE"), true);
      return false;
    }
