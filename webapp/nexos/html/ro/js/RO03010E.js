...
function _Inquiry() {
  ...
  if ($NC.G_VAR.activeView.PROCESS_CD === "B") {
    ...
    // 데이터 조회
    $NC.serviceCall("/RO03010E/getDataSet.do", $NC.getGridParams(G_GRDMASTERB), onGetMasterB);
  ...
  if ($NC.G_VAR.activeView.PROCESS_CD === "D") {
    ...
    // 데이터 조회
    $NC.serviceCall("/RO03010E/getDataSet.do", $NC.getGridParams(G_GRDMASTERD), onGetMasterD);
  }
}
...
function _Save() {
  ...
  if ($NC.G_VAR.activeView.PROCESS_CD === "D") {
    ...
    if (saveDS.length > 0 || mstRow.CRUD !== "R") {
      $NC.serviceCall("/RO03010E/saveConfirms.do", {
...
function setMasterSummaryInfo() {
  ...
  // 데이터 조회
  $NC.serviceCall("/RO03010E/getDataSet.do", {
    P_QUERY_ID: "RO03010E.RS_MASTER",
    P_QUERY_PARAMS: $NC.getParams({
      P_CENTER_CD: CENTER_CD,
      P_BU_CD: BU_CD,
      P_OUTBOUND_DATE: OUTBOUND_DATE,
...
function setPolicyValInfo() {
  ...
  for ( var POLICY_CD in $NC.G_VAR.policyVal) {
    // 데이터 조회
    $NC.serviceCall("/RO03010E/callSP.do", {
