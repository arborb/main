...
function _Initialize() {
  ...
  // 조회조건 - 반출일자 초기화
  $NC.setInitDatePicker("#dtpQOutbound_Date1", null, "F");
...
function _Inquiry() {
  ...
  if (OUTBOUND_DATE1 > OUTBOUND_DATE2) {
    alert("반출일자 검색 범위 오류입니다.");
    $NC.setFocus("#dtpQOurbound_Date1");
    return;
  }
  ...
  if ($("#divMasterView").tabs("option", "active") === 0) {
    ...
    // 데이터 조회
    $NC.serviceCall("/RO03030Q/getDataSet.do", $NC.getGridParams(G_GRDMASTERT1), onGetMasterT1);
    ...
    // 데이터 조회
    $NC.serviceCall("/RO03030Q/getDataSet.do", $NC.getGridParams(G_GRDMASTERT2), onGetMasterT2);
