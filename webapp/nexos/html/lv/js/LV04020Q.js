...
function _Inquiry() {
  ...
  // 주간, 월간, 분기, 연간
  $NC.serviceCall("/LV04020Q/getDataSet.do", {
    P_QUERY_ID: "LV04020Q.MASTER_REPORT",
    P_QUERY_PARAMS: $NC.getParams({
    ...
    for (var i = 0, resultCount = resultData.length; i < resultCount; i++) {
      ...
      case "WEEK" :
        if (rowData.YEAR_TYPE == "CURRENT") {
          seriesDS[0][0].push(rowData.OUT_QTY);
        } else if (rowData.YEAR_TYPE == "PREV") {
          seriesDS[0][1].push(rowData.OUT_QTY);
        }
        if ($.inArray(rowData.OUT_DATE, ticks[0]) < 0) {
          ticks[0].push(rowData.OUT_DATE);
        }
        break;
      case "MONTH" :
        if (rowData.YEAR_TYPE == "CURRENT") {
          seriesDS[1][0].push(rowData.OUT_QTY);
        } else if (rowData.YEAR_TYPE == "PREV") {
          seriesDS[1][1].push(rowData.OUT_QTY);
        }
        if ($.inArray(rowData.OUT_DATE, ticks[1]) < 0) {
          ticks[1].push(rowData.OUT_DATE);
        }
        break;
      case "QUARTER" :
        if (rowData.YEAR_TYPE == "CURRENT") {
          seriesDS[2][0].push(rowData.OUT_QTY);
        } else if (rowData.YEAR_TYPE == "PREV") {
          seriesDS[2][1].push(rowData.OUT_QTY);
        }
        if ($.inArray(rowData.OUT_DATE, ticks[2]) < 0) {
          ticks[2].push(rowData.OUT_DATE);
        }
        break;
      case "YEAR" :
        if (rowData.YEAR_TYPE == "CURRENT") {
          seriesDS[3][0].push(rowData.OUT_QTY);
        } else if (rowData.YEAR_TYPE == "PREV") {
          seriesDS[3][1].push(rowData.OUT_QTY);
        }
        if ($.inArray(rowData.OUT_DATE, ticks[3]) < 0) {
          ticks[3].push(rowData.OUT_DATE);
        }
        break;
      }
    }
