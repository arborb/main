/*
 * IE7/IE8/IE9.js - copyright 2004-2010, Dean Edwards http://code.google.com/p/ie7-js/
...
/*
 * credits/thanks: Shaggy, Martijn Wargers, Jimmy Cerra, Mark D Anderson, Lars Dieckow, Erik Arvidsson, Gell�rt Gyuris,
 * James Denny, Unknown W Brackets, Benjamin Westfarer, Rob Eberhardt, Bill Edney, Kevin Newman, James Crompton, Matthew
 * Mastracci, Doug Wright, Richard York, Kenneth Kolano, MegaZone, Thomas Verelst, Mark 'Tarquin' Wilton-Jones, Rainer
 * �hlfors, David Zulaica, Ken Kolano, Kevin Newman, Sjoerd Visscher, Ingo Chao
 */
...
(function(window, document) {
  ...
  var REGGRP_BACK_REF = /\\(\d+)/g, REGGRP_ESCAPE_COUNT = /\[(\\.|[^\]\\])+\]|\\.|\(\?/g, REGGRP_PAREN = /\(/g, REGGRP_LOOKUP = /\$(\d+)/, REGGRP_LOOKUP_SIMPLE = /^\$\d+$/, REGGRP_LOOKUPS = /(\[(\\.|[^\]\\])+\]|\\.|\(\?)|\(/g, REGGRP_DICT_ENTRY = /^<#\w+>$/, REGGRP_DICT_ENTRIES = /<#(\w+)>/g;
  ...
  var RegGrp = Base.extend({
    ...
    exec: function(string) {
      var group = this, patterns = group[KEYS], items = group[ITEMS], item;
        ...
        while ((item = items[HASH + patterns[i++]])) {
          ...
          if (result[offset]) { // do we have a result?
            if (item.replacement === 0) {
              ...
              while (--j)
                args[j] = args[j] || ""; // some platforms return null/undefined for non-matching sub-expressions
              args[0] = {
                match: args[0],
                item: item
              };
    ...
    parse: function(string) {
      string += ""; // type safe
      var group = this, patterns = group[KEYS], items = group[ITEMS];
      return string.replace(this.compile(), function(match) {
        var args = [ ], item, offset = 1, i = arguments.length;
        while (--i)
          args[i] = arguments[i] || ""; // some platforms return null/undefined for non-matching sub-expressions
        // Loop through the RegGrp items.
        while ((item = items[HASH + patterns[i++]])) {
          var next = offset + item.length + 1;
    ...
    toString: function() {
      var strings = [ ], keys = this[KEYS], items = this[ITEMS], item;
  ...
  var SINGLE_QUOTES = /'/g, ESCAPED = /'(\d+)'/g, ESCAPE = /\\/g, UNESCAPE = /\\([nrtf'"])/g;
  ...
  function encodeString(string) {
    ...
    strings[index] = string.slice(1, -1).replace(UNESCAPE, "$1").replace(SINGLE_QUOTES, "\\'");
    return "'" + index + "'";
  }
  ;
  ...
  function register(handler, element, condition) { // -@DRE
    ...
    if (condition) handler.elements[element.uniqueID] = element;
      ...
      delete handler.elements[element.uniqueID];
  ...
  var NEXT_SIBLING = "(e.nextSibling&&IE7._getElementSibling(e,'next'))", PREVIOUS_SIBLING = NEXT_SIBLING.replace(
      /next/g, "previous"), IS_ELEMENT = "e.nodeName>'@'", IF_ELEMENT = "if(" + IS_ELEMENT + "){";
  ...
  IE7.CSS = new (Fix.extend({ // single instance
    ...
    init: function() {
      ...
      dynamicPseudoClasses = "not\\(:" + dynamicPseudoClasses.split("|").join("\\)|not\\(:") + "\\)|"
          + dynamicPseudoClasses;
  ...
  var StyleSheet = Base.extend({
    ...
    getText: function() {
      ...
      function loadStyleSheet(styleSheet, path) {
        ...
        // Load from source
        fileCache[url] = styleSheet.disabled ? "" : fixUrls(IE7.CSS.getText(styleSheet, path), getPath(href, path));
  ...
  addEventHandler(document, "onmouseup", function() {
    ...
    for ( var i in instances)
      if (!instances[i][0].contains(event.srcElement)) Hover.unregister(instances[i]);
  });
  ...
  (function() {
    ...
    layout.boxSizing = function(element) {
      if (!element.currentStyle.hasLayout) {
        ...
        if (element.currentStyle.verticalAlign === "auto") element.runtimeStyle.verticalAlign = "top";
    ...
    function collapseMargin(element, type) {
      if (!element.runtimeStyle[type]) {
        ...
        if (parentElement && parentElement.currentStyle.hasLayout
            && !IE7._getElementSibling(element, isTopMargin ? "previous" : "next")) return;
    ...
    var _fixWidth = function(HEIGHT) {
      ...
      function _fixWidth(element, value) {
        if (!element.runtimeStyle.fixedWidth) {
          ...
          element.runtimeStyle.fixedWidth = UNIT.test(value) ? Math.max(0, getFixedWidth(element, value)) + "px"
              : value;
      ...
      function layoutWidth(element) {
        if (!isFixed(element)) {
          ...
          while (layoutParent && !layoutParent.currentStyle.hasLayout)
            layoutParent = layoutParent.offsetParent;
        }
      ...
      var getFixedWidth = function(element, value) {
        ...
        var adjustment = 0;
        if (MSIE5 && !borderBox) adjustment += getBorderWidth(element) + getWidth(element, "padding");
        else if (!MSIE5 && borderBox) adjustment -= getBorderWidth(element) + getWidth(element, "padding");
      ...
      function getWidth(element, type) {
        return getPixelWidth(element, element.currentStyle[type + "Left"])
            + getPixelWidth(element, element.currentStyle[type + "Right"]);
      }
      ;
      ...
      function resizeWidth(element) {
        ...
        } else if (element.currentStyle.maxWidth !== "none"
            && width >= getFixedWidth(element, element.currentStyle.maxWidth)) {
      ...
      function resizeRight(element) {
        ...
        var width = layoutWidth(element) - getPixelWidth(element, element.currentStyle.right) - left
      ...
      addResize(function() {
        ...
        for (i in elements) {
          ...
          var fixedWidth = (parseInt(element.runtimeStyle.width) === getFixedWidth(element,
              element.currentStyle.minWidth));
        ...
        for (i in elements) {
          var element = elements[i];
          var fixedWidth = (parseInt(element.runtimeStyle.width) === getFixedWidth(element,
              element.currentStyle.maxWidth));
          if (!wider && fixedWidth) element.runtimeStyle.width = "";
          if (wider !== fixedWidth) resizeWidth(element);
        }
        // resize elements with "right" set
        for (i in fixRight.elements)
          resizeRight(fixRight.elements[i]);
      });
      ...
      if (appVersion < 7) {
        ...
        if (HEIGHT)
          IE7.CSS.addRecalc("height", "[\\d.]+%", function(element) {
  ...
  if (appVersion < 7) {
    ...
    IE7.HTML.addRecalc("img,input", function(element) {
      ...
      addEventHandler(element, "onpropertychange", function() {
        if (!printing && event.propertyName === "src" && element.src.indexOf(BLANK_GIF) === -1) fixImage(element);
      });
    });
  ...
  (function() {
    ...
    function _setExpression(element, propertyName, expression) {
      setTimeout("document.all." + element.uniqueID + ".runtimeStyle.setExpression('" + propertyName + "','"
          + expression + "')", 0);
    }
    ;
    ...
    function _backgroundFixed(element) {
      if (register(_backgroundFixed, element, element.currentStyle.backgroundAttachment === "fixed"
    ...
    function _resize() {
      ...
      for ( var i in elements)
        _backgroundPosition(elements[i]);
    ...
    var _horizontal = function(util) {
      util.bgLeft = function(element) {
        ...
        if (!_isFixed(element)) {
          _setExpression(element, "backgroundPositionX", "(parseInt(runtimeStyle.offsetLeft)+document." + $viewport
              + ".scrollLeft)||0");
        }
      };
      ...
      util.setOffsetLeft = function(element) {
        ...
        element.runtimeStyle[propertyName] = util.getOffsetLeft(element, element.style.backgroundPositionX)
      ...
      util.positionLeft = function(element, recalc) {
        ...
        if (!recalc && !_isFixed(element.offsetParent)) {
          ...
          _setExpression(element, "pixelLeft", "runtimeStyle.screenLeft+runtimeStyle.shiftLeft+document." + $viewport
              + ".scrollLeft");
        }
      };
      ...
      util.getPixelWidth = function(element, value) {
        return PERCENT.test(value) ? parseInt(parseFloat(value) / 100 * viewport.clientWidth) : getPixelValue(element,
            value);
      };
    };
  ...
  function ie7Quirks() {
    ...
    IE7.CSS.addRecalc("margin(-left|-right)?", "[^};]*auto", function(element) {
      if (register(fixMargin, element, element.parentElement && element.currentStyle.display === "block"
          && element.currentStyle.marginLeft === "auto" && element.currentStyle.position !== "absolute")) {
        fixMargin(element);
      }
    });
  ...
  if (appVersion < 8) {
    ...
    function parseInherited(cssText) {
      return cssText.replace(new RegExp("([{;\\s])(" + inheritedProperties.join("|") + ")\\s*:\\s*([^;}]+)", "g"),
          "$1$2:$3;ie7-$2:$3");
    }
    ;
    ...
    var PseudoElement = IE7.PseudoElement = Rule.extend({
      ...
      init: function() {
        ...
        for ( var i = 0; i < this.match.length; i++) {
          ...
          if (!runtimeStyle[this.position]) runtimeStyle[this.position] = {
            cssText: ""
          };
      ...
      create: function(target) {
        ...
        if (generated) {
          ...
          target.insertAdjacentHTML(position, format(PseudoElement.ANON, this.className, id, cssText, url ? ""
              : content));
          if (url) {
  ...
  var NOT_NEXT_BY_TYPE = "!IE7._getElementSiblingByType(e,'next')&&", NOT_PREVIOUS_BY_TYPE = NOT_NEXT_BY_TYPE.replace(
      "next", "previous");
  ...
  IE7._indexOf = function(element, last, ofType) {
    ...
    var id = (parent.uniqueID) + "-" + tagName, indexes = allIndexes[id];
    if (!indexes) {
      indexes = {};
      var index = 0, child = parent.firstChild;
      while (child) {
  ...
  FILTER = extend(extend({
    ":nth(-last)?-(?:child|(of-type))\\((<#nth_arg>)\\)(<#filter>)?": function(match, last, ofType, args, filters) { // :nth-
                                                                                                                      // pseudo
                                                                                                                      // classes
      ...
      args = args.split("n");
      var a = ~~(ONE[args[0]] || args[0] || 1), b = ~~args[1];
      if (a === 0) {
        var expr = index + "===" + b;
      } else {
  ...
  var HTML5 = "article,aside,audio,canvas,details,figcaption,figure,footer,header,hgroup,mark,menu,meter,nav,output,progress,section,summary,time,video"
      .split(",");
...
mark{background:yellow}";
  ...
  var cssQuery = (function() {
    ...
    var IMPLIED_ASTERISK = /(^|[, >+~])([#.:\[])/g, BLOCKS = /\)\{/g, COMMA = /,/, QUOTED = /^['"]/, HEX_ESCAPE = /\\([\da-f]{2,2})/gi, LAST_CHILD = /last/i;
    ...
    function normalize(selector) {
      selector = normalizer.parse(selector.replace(HEX_ESCAPE, "\\x$1")).replace(UNESCAPE, "$1").replace(
          IMPLIED_ASTERISK, "$1*$2");
    ...
    var BY_ID = "e0=IE7._byId(d,'%1');if(e0){", BY_TAG_NAME = "var n=c.getElementsByTagName('%1');", STORE = "if(r==null)return e0;r[k++]=e0;";
    ...
    var selectQuery = new CSSSelectorParser({
      "<#grammar>": function(match, selector, remainingSelectors) {
        ...
        var groups = this.groups, tagName = groups[0][TAG_NAME]; // first tag name
        ...
        var isWild = tagName === "*";
        return (isWild ? "var n=c.all;" : format(BY_TAG_NAME, tagName)) + "filtering:while((e0=n[i++]))"
            + (isWild ? IF_ELEMENT.replace(TOKEN, "e0") : "{") + matcher + "}";
      },
    ...
    selectQuery.create = function(selector) {
      if (!cache[selector]) {
        ...
        if (selector.indexOf("#") !== -1) {
          ...
          if (byId) {
            block = "if(t===1||t===11|!c.getElementById){" + block + "}else{" + byId + "}";
          }
        }
