 ...
 */
/**
 * The splitter() plugin implements a two-pane resizable splitter window. The selected elements in the jQuery object are
 * converted to a splitter; each selected element should have two child elements, used for the panes of the splitter.
 * The plugin adds a third child element for the splitbar. For more details see: http://methvin.com/jquery/splitter/
 * 
 ...
 * @type jQuery
 * @param Object
 *          options Options for the splitter (not required)
  ...
  $.fn.splitter = function(args) {
    ...
    return this
        .each(function() {
          ...
          var outerlineResizer; // left-behind splitbar for outline resizes

          function resizeAutoFired() {
            // Returns true when the browser natively fires the resize
            // event attached to the panes elements
            return ($.browser.msie && ($.browser.versionNumber < 9));
          }
          ...
          function startSplitMouse(evt) {
            ...
            if (opts.outline) {
              outerlineResizer = outerlineResizer || bar.clone(false).insertAfter(A);
            ...
            $(document).bind("mousemove" + opts.eventNamespace, doSplitMouse).bind("mouseup" + opts.eventNamespace,
                endSplitMouse);
          }

          function doSplitMouse(evt) {
            var pos = A._posSplit + evt[opts.eventPos], range = Math.max(0, Math.min(pos, splitter._DA - bar._DA)), limit = Math
                .max(A._min, splitter._DA - B._max, Math.min(pos, A._max, splitter._DA - bar._DA - B._min));
            if (opts.outline) {
          ...
          function endSplitMouse(evt) {
            ...
            if (opts.outline) {
              outerlineResizer && outerlineResizer.remove();
              outerlineResizer = null;
              resplit(pos);
            }
          ...
          function resplit(pos) {
            ...
            A.css(opts.origin, 0).css(opts.split, pos).css(opts.maxSplit, pos).css(opts.fixed, splitter._DF).css(
                opts.maxFixed, splitter._DF);
            B.css(opts.origin, pos + bar._DA).css(opts.split, splitter._DA - bar._DA - pos).css(opts.maxSplit,
                splitter._DA - bar._DA - pos).css(opts.fixed, splitter._DF).css(opts.maxFixed, splitter._DF);
            // IE fires resize for us; all others pay cash
            if (!resizeAutoFired()) {
              splitter.trigger("resize", [pos]);
            }
          }
          ...
          function resize(size) {
            ...
            if (splitter._oldW == splitter.width() && splitter._oldH == splitter.height()
                && splitter._oldB == parseInt(bar.css(opts.origin))) {
            ...
            resplit(!isNaN(size) ? size : (!(opts.sizeRight || opts.sizeBottom) ? A[0][opts.pxSplit] : splitter._DA
                - B[0][opts.pxSplit] - bar._DA));
            setBarState(opts.barNormalClass);
          }
          ...
          var opts = $.extend({
            ...
            eventNamespace: ".splitter" + new Date().getTime() + Math.floor((Math.random() * 10000) + 1),// (++splitterCounter),
          ...
          opts.barStateClasses = [opts.barNormalClass, opts.barHoverClass, opts.barActiveClass, opts.barLimitClass]
              .join(' ');
          ...
          // Splitbar element
          var bar = $('<div></div>').insertAfter(A).addClass(opts.barClass).addClass(opts.barStateClass)
              .append(focuser).attr({
                unselectable: "on"
              }).css({
                position: "absolute",
                "z-index": "100"
              }).bind("mousedown" + opts.eventNamespace, startSplitMouse).bind("mouseover" + opts.eventNamespace,
                  function() {
          ...
          $.each([A, B], function() {
            ...
            this._init = opts["size" + this._pane] === true ? parseInt($.css(this[0], opts.split), 200) : opts["size"
                + this._pane];
          });
          ...
          if (opts.dock) {
            splitter.bind("toggleDock" + opts.eventNamespace, function() {
              ...
              splitter.triggerHandler(pw ? "dock" : "undock");
                ...
                "dock" + opts.eventNamespace,
                function() {
                  var pw = A[0][opts.pxSplit];
                  if (!pw) return;
                  bar._pos = pw;
                  var x = {};
                  x[opts.origin] = opts.dockPane == A ? 0 : splitter[0][opts.pxSplit] - splitter._PBA
                      - bar[0][opts.pxSplit];
                ...
                "undock" + opts.eventNamespace,
                function() {
                  var pw = opts.dockPane[0][opts.pxSplit];
                  if (pw) return;
                  var x = {};
                  x[opts.origin] = bar._pos + "px";
                  bar.removeClass(opts.barDockedClass).animate(x, opts.undockSpeed || opts.dockSpeed || 1,
            ...
            if (opts.dockKey)
              $('<a title="' + opts.splitbarClass + ' toggle dock" href="javascript:void(0)"></a>').attr({
          ...
          splitter.bind("resize" + opts.eventNamespace, function(e, size) {
            resize(!isNaN(size) ? size : splitter._oldB);
          });
