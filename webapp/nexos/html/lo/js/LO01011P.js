...
function _Initialize() {
  ...
  $NC.setGlobalVar({
    // 마스터 데이터
    masterData: null,
    onStockSelInfoViewTimeout: null
  });
  ...
  $("#btnOwn_Brand_Cd").click(showOwnBranPopup); // 위탁사 검색 버튼
  $("#btnDelivery_Cd").click(showDeliveryPopup); // 배송처 검색 버튼
  $("#btnRDelivery_Cd").click(showRDeliveryPopup); // 실배송처 검색 버튼
...
function _OnPopupOpen() {
  ...
  if ($NC.G_VAR.userData.P_PROCESS_CD === "N") {
    ...
    $NC.setValue("#cboOrder_Div", masterDS.ORDER_DIV);
...
function _OnInputChange(e, view, val) {
  ...
  // 출고예정등록/수정 팝업에서 [제고에서선택] 버튼 클릭해서 표시된 팝업창에서의 검색조건 변경시
...
//  var BRAND_CD = $NC.getValue("#edtQBrand_Cd", true);
  ...
  // 데이터 조회
  $NC.serviceCall("/LO01010E/getDataSet.do", $NC.getGridParams(G_GRDSTOCKSELMASTER), onGetStockSelMaster);
...
function _Save() {
  ...
  $NC.serviceCall("/LO01010E/save.do", {
...
function gridDetailCheckPSTOCK(row, isCheckOnly) {
  ...
  if (rowData) {
    // 데이터 조회
    $NC.serviceCall("/LO01010E/callSP.do", {
