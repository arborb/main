...
function _SetResizeOffset() {
  ...
  $NC.G_OFFSET.nonClientHeight = $NC.G_LAYOUT.nonClientHeight + 100;
  $NC.G_OFFSET.rightViewMinHeight = $("#divMasterInfoView").outerHeight(true) + $NC.G_LAYOUT.header
      + $NC.G_OFFSET.nonClientHeight + $NC.G_LAYOUT.margin2 + 100;
}
...
function _Inquiry() {
  ...
  // 데이터 조회
  $NC.serviceCall("/LOM3060E/getDataSet.do", $NC.getGridParams(G_GRDMASTER), onGetMaster);
}
...
function setInputValue(grdSelector, rowData) {
  ...
  if (grdSelector === "#grdMaster") {
    ...
    // Row 데이터로 에디터 세팅
    $NC.setValue("#edtZip_Cd", rowData["SHIPPER_ZIP_CD"]);
    $NC.setValue("#edtAddr_Basic", rowData["SHIPPER_ADDR_BASIC"]);
    $NC.setValue("#edtAddr_Detail", rowData["SHIPPER_ADDR_DETAIL"]);
    ...
    case "ZIP_CD":
      ...
      if (!$NC.isNull(args.val)) {
        ...
        O_RESULT_DATA = $NP.getPostInfo({
          queryParams: P_QUERY_PARAMS,
          queryCanAll: true,
          errorMessage: "등록되어 있지 않은 우편번호입니다."
        });
      }
      if (O_RESULT_DATA.length <= 1) {
      ...
      return;
    case "ADDR_BASIC":
      rowData.SHIPPER_ADDR_BASIC = args.val;
      break;
    case "ADDR_DETAIL":
      rowData.SHIPPER_ADDR_DETAIL = args.val;
      break;
    case "SHIPPER_HP":
      rowData.SHIPPER_HP = args.val;
      break;
    case "SHIPPER_NM":
      rowData.SHIPPER_NM = args.val;
      break;
    }
...
function onPostPopup(resultInfo) {
  ...
  if (!$NC.isNull(resultInfo)) {
    ...
    rowData.SHIPPER_ZIP_CD = resultInfo.ZIP_CD;
    rowData.SHIPPER_ADDR_BASIC = resultInfo.ADDR_NM_REAL;
  }
