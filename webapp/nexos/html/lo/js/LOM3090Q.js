...
function Deal_Ch() {
  ...
  var DEAL_ID = $NC.getValue("#edtQCh_Deal");
  if ($NC.isNull(DEAL_ID)) {
    alert("변경 할 대상 딜 을 입력하세요.");
    $NC.setFocus("#edtQCh_Deal");
    return;
  }

  var DEALITEM1 = $NC.getValue("#edtQCh_DealItem1");
  if ($NC.isNull(DEALITEM1)) {
    alert("변경전 상품 코드를 입력하세요.");
    $NC.setFocus("#edtQCh_DealItem1");
    return;
  }
  var DEALITEM2 = $NC.getValue("#edtQCh_DealItem2");
  if ($NC.isNull(DEALITEM2)) {
    alert("변경후 상품 코드를 입력하세요.");
    $NC.setFocus("#edtQCh_DealItem2");
    return;
  }
  ...
  for ( var row = 0; row < rowCount; row++) {
    ...
    if (rowData.CHECK_YN == "Y") {
        ...
        var Ms = {
          ...
          P_ORDER_NO: rowData.ORDER_NO,
          P_DEAL_ID: DEAL_ID,
          P_ITEM_CD_FROM: DEALITEM1,
          P_ITEM_CD_TO: DEALITEM2,
        };
        closingDS.push(Ms);
    }
  }
  if (chkCnt == 0) {
    ...
    $NC.serviceCall("/LOM3090Q/callDealItemUpdate.do", {
