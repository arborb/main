...
function _Initialize() {
  ...
  $NC.G_VAR.USER_DATA = $NC.G_JWINDOW.get("userData");
  $NC.G_VAR.OUTBOUND_DATE = $NC.G_VAR.USER_DATA.OUTBOUND_DATE;
  $NC.G_VAR.TT_DIV = $NC.G_VAR.USER_DATA.TEST;
  // 추가 조회조건 사용
...
function _Inquiry() {
  ...
  if ($NC.G_VAR.TT_DIV == "1"){
   var TT1 = $NC.G_VAR.OUTBOUND_DATE;
   $NC.setValue("#dtpQOutbound_Date",TT1);   
 }
...
function _Print(printIndex, printName) {
  ...
  // 출력 데이터 Array 담기
  // 지시서, 거래명세서
  if (printIndex == 0 || printIndex == 1 || printIndex == 2 || printIndex == 3 || printIndex == 4 || printIndex == 8 || printIndex == 9) {
  ...
  if (printIndex == 0 || printIndex == 8 || printIndex == 9) {
    // 선택 건수 체크
    if (checkCnt === 0) {
