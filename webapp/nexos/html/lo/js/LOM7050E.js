...
function _Inquiry() {
  ...
  // 데이터 조회
  $NC.serviceCall("/LOM7050E/getDataSet.do", $NC.getGridParams(G_GRDMASTER), onGetMaster);
}
...
function onInspectUpdateProc(center_Cd, bu_Cd, order_Date, order_No) {

  $NC.serviceCall("/LOM7050E/callInspectProc.do", {
    P_QUERY_ID: "LO_SCAN_LT_STEP2",
    P_QUERY_PARAMS: $NC.getParams({
      P_CENTER_CD: center_Cd,
      P_BU_CD: bu_Cd,
      P_ORDER_DATE: order_Date,
...
function onGetMaster(ajaxData) {
  ...
  if (rowData.INSPECT_YN == "Y"){
    $NC.setValue("#edtInspect_Chk", "검수완료");
    $("#edtInspect_Chk").addClass("inspected");
    setFocusScan();
    return;
  } else {
    $NC.setValue("#edtInspect_Chk", "미검수");
    $("#edtInspect_Chk").removeClass("inspected");
    onInspectUpdateProc(rowData.CENTER_CD, rowData.BU_CD, rowData.ORDER_DATE, rowData.ORDER_NO);
  }
