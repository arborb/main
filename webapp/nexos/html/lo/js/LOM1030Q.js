...
function _Initialize() {
  ...
  // 미출예상상품 사업부이관 버튼
  $("#btnProcChangeBu").click(onBtnProcChangeBuClick);
...
function _Inquiry() {
  ...
  // 데이터 조회
  $NC.serviceCall("/LOM1030Q/getDataSet.do", $NC.getGridParams(G_GRDMASTER), onGetMaster);
}
...
function onGetMaster(ajaxData) {
  ...
  // print 버튼 활성화 처리
  if (G_GRDMASTER.data.getLength() > 0) {
    $NC.setEnable("#btnProcChangeBu", true);
  }
}
...
function onBtnProcChangeBuClick() {

  var result = confirm("미출예상상품 사업부이관를 처리하시겠습니까?");
  if (!result) {
    return;
  }
  ...
  $NC.serviceCall("/LOM1030Q/callChangeBu.do", {
