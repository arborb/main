  ...
  case "SCAN":
    ...
    if (e.keyCode == 13) {
      ...
      if (scanVal.substr(0, 2) == "OP" && scanVal.match(/-/g).length == 3) {
        ...
        e.stopImmediatePropagation();
        alert("주문확인서번호를 확인하시고 다시 스캔해주십시오.");
        setFocusScan();
        return;
      }
...
function _Inquiry() {
  ...
  // 데이터 조회
  $NC.serviceCall("/LOM7040E/getDataSet.do", $NC.getGridParams(G_GRDMASTER), onGetMaster);
}
...
function grdMasterInitialize() {
  ...
  $NC.setInitGridObject("#grdMaster", {
    ...
    queryId: "LOM7040E.RS_MASTER",
    sortCol: ["SELLER_CD","ITEM_CD"],
    gridOptions: options
  });
...
function setItemInfoValue(rowData) {
  ...
  $NC.setValue("#edtLt_Inspect_Yn", rowData.LT_INSPECT_YN);
...
function onWbUpdateProc(center_Cd, bu_Cd, order_Date, order_No) {

  $NC.serviceCall("/LOM7040E/callWbProc.do", {
    P_QUERY_ID: "LO_SCAN_LT_STEP1",
    P_QUERY_PARAMS: $NC.getParams({
      P_CENTER_CD: center_Cd,
      P_BU_CD: bu_Cd,
      P_ORDER_DATE: order_Date,
...
function onGetMaster(ajaxData) {
  ...
  if (rowData.LT_INSPECT_YN == "Y"){
    $NC.setValue("#edtInspect_Chk", "등록완료");
    $("#edtInspect_Chk").addClass("inspected");
    setFocusScan();
    return;
  } else {
    $NC.setValue("#edtInspect_Chk", "미등록");
    $("#edtInspect_Chk").removeClass("inspected");
    onWbUpdateProc(rowData.CENTER_CD, rowData.BU_CD, rowData.ORDER_DATE, rowData.ORDER_NO);
  }
