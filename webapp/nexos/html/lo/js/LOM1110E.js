...
function _Initialize() {
  ...
  // 진행상태 콤보박스 세팅
  var cboObj = $("#cboQPrint_Yn").empty();
  var optionStr = "";
  optionStr += "<option value='%'>% - 전체</option>";
  optionStr += "<option value='Y'>Y - 출력</option>";
  optionStr += "<option value='N'>N - 미출력</option>";
  cboObj.append(optionStr);
  $NC.setValue("#cboQPrint_Yn", 0);
...
function _OnConditionChange(e, view, val) {
  ...
  case "ORDER_DATE1":
    ...
    // 조회조건 - 정산항목 세팅
    $NC.setInitCombo("/L0M1110E/getDataSet.do", {
      P_QUERY_ID: "L0M1110E.ORDER_PRINT_DATE",
      P_QUERY_PARAMS: $NC.getParams({
        P_ORDER_DATE1: $NC.getValue("#dtpQOrder_Date1"),
        P_ORDER_DATE2: $NC.getValue("#dtpQOrder_Date2"),
      })
    }, {
      selector: "#cboQPrint_Date",
      codeField: "ORDER_DATE",
      nameField: "ORDER_DATE_T",
      fullNameField: "ORDER_DATE",
      addAll: true,
      onComplete: function() {
        // 조회조건 - 정산세부 세팅
  ...
  case "ORDER_DATE2":
    ...
    // 조회조건 - 정산항목 세팅
    $NC.setInitCombo("/L0M1110E/getDataSet.do", {
      P_QUERY_ID: "L0M1110E.RS_ORDER_PRINT_DATE",
      P_QUERY_PARAMS: $NC.getParams({
        P_ORDER_DATE1: $NC.getValue("#dtpQOrder_Date1"),
        P_ORDER_DATE2: $NC.getValue("#dtpQOrder_Date2"),
      })
...
function _Inquiry() {
  ...
  // 데이터 조회
  $NC.serviceCall("/LOM1110E/getDataSet.do", $NC.getGridParams(G_GRDMASTER), onGetMaster);
...
function saveLogic() {
  ...
  saveDS.push(saveData);
  if (saveDS.length > 0) {
    $NC.serviceCall("/LOM1110E/delete.do", {
      P_DS_MASTER: $NC.toJson(saveDS),
      P_USER_ID: $NC.G_USERINFO.USER_ID
    }, onSave, onSaveError);
  }
...
function _Print(printIndex, printName) {
  ...
  for ( var row = 0; row < rowCount; row++) {
    ...
    if (rowData.CHECK_YN === "Y") {
      checkCnt++;
      checkedValueDS.push(rowData.BU_NO);
      var saveData = {
        P_CENTER_CD: rowData.CENTER_CD,
        P_BU_CD: rowData.BU_CD,
        P_ORDER_DATE: rowData.ORDER_DATE,
  ...
  $NC.serviceCallAndWait("/LOM1110E/savePrintHis.do", {
    P_DS_DETAIL: $NC.toJson(saveDs),
    P_USER_ID: $NC.G_USERINFO.USER_ID
  }, onSavePrintYn);

  var reportDoc = "lo/PAPER_LOM01_4";
  var queryId = "WR.RS_PAPER_LOM09";
  var queryParams = {
    P_CENTER_CD: CENTER_CD,
    P_BU_CD: BU_CD,
    P_ORDER_DATE1: ORDER_DATE1,
...
function onSavePrintYn(ajaxData) {
  ...
  if (!$NC.isNull(resultData)) {
    ...
    } else if (resultData.RESULT_DATA == "OK") {
      var CENTER_CD = $NC.getValue("#cboQCenter_Cd");
      var BU_CD = $NC.getValue("#edtQBu_Cd");

      $NC.serviceCall("/LOM1110E/callSP.do", {
        P_QUERY_ID: "LO_LOTTE_BATCH",
        P_QUERY_PARAMS: $NC.getParams({
          P_CENTER_CD: CENTER_CD,
          P_BU_CD: BU_CD,
          P_USER_ID: $NC.G_USERINFO.USER_ID
...
function grdMasterOnAfterScroll(e, args) {
  ...
  // 데이터 조회
  $NC.serviceCall("/LOM1110E/getDataSet.do", $NC.getGridParams(G_GRDDETAIL), onGetDetail);
...
function setOutboundBatchCombo(comboId, isAddAll, setPos) {
  ...
  // 조회조건 - 출고차수 세팅
  $NC.setInitCombo("/WC/getDataSet.do", {
    P_QUERY_ID: "WC.POP_LOTTE_ORDER_BATCH",
    P_QUERY_PARAMS: $NC.getParams({
    ...
    selector: comboId,
    codeField: "ORDER_BATCH",
    nameField: "ORDER_BATCH",
    fullNameField: "ORDER_BATCH_F",
    addAll: true,
