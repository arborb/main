...
function _OnPopupOpen() {
  ...
  if ($NC.G_VAR.userData.P_PROCESS_CD === "N") {
    ...
    $NC.setValue("#edtRDelivery_Cd", $NC.G_VAR.masterData.RDELIVERY_CD);
    $NC.setValue("#edtRDelivery_Nm", $NC.G_VAR.masterData.RDELIVERY_NM);
    ...
    $NC.setEnable("#edtRDelivery_Cd", false);
    $NC.setEnable("#btnRDelivery_Cd", false);
  ...
  $NC.setInitCombo("/WC/getDataSet.do", {
    ...
    onComplete: function() {
      if ($NC.G_VAR.userData.P_PROCESS_CD == "N") {
        $NC.G_VAR.masterData.ORDER_DIV = $NC.getValue("#cboOrder_Div");
      } else {
        $NC.setValue("#cboOrder_Div", $NC.G_VAR.masterData.ORDER_DIV);
      }
    }
...
function _OnInputChange(e, view, val) {
  ...
  // 출고등록/수정 팝업에서 [제고에서선택] 버튼 클릭해서 표시된 팝업창에서의 검색조건 변경시
...
//  var BRAND_CD = $NC.getValue("#edtQBrand_Cd", true);
  var BRAND_CD = $NC.getValue("#edtOwn_Brand_Cd", true);
  ...
  // 데이터 조회
  $NC.serviceCall("/LO02010E/getDataSet.do", $NC.getGridParams(G_GRDSTOCKSELMASTER), onGetStockSelMaster);
...
function _Save() {
  ...
  if ($NC.isNull($NC.G_VAR.masterData.ORDER_DIV)) {
    $NC.G_VAR.masterData.INOUT_CD = $NC.getValue("#cboOrder_Div");
  }
  // 현재 수정모드면
  if (G_GRDDETAIL.view.getEditorLock().isActive()) {
    G_GRDDETAIL.view.getEditorLock().commitCurrentEdit();
  ...
  $NC.serviceCall("/LO02010E/save.do", {
    P_DS_MASTER: $NC.toJson({
      ...
      P_ORDER_DIV: $NC.getValue("#cboOrder_Div"),
...
function showStockOverlay(e) {
  ...
  if (rowData) {
    ...
    // 데이터 조회
    $NC.serviceCallAndWait("/LO02010E/getDataSet.do", $NC.getGridParams(G_GRDSTOCKMASTER), onGetStockMaster);
...
function showStockSelOverlay(e) {
  ...
  var DELIVERY_CD = $NC.getValue("#edtDelivery_Cd");
  var OWN_BRAND_CD = $NC.getValue("#edtOwn_Brand_Cd");
  if ($NC.isNull(DELIVERY_CD) || $NC.isNull(OWN_BRAND_CD)) {
    alert("먼저 위탁사와 거래처를 입력하십시오.");
    $NC.setFocus("#edtOwn_Brand_Cd");
    return;
  }
