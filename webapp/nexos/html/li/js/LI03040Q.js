...
//  $NC.setInitDatePicker("#dtpQInbound_Date1", null, "F"); // 당월의 첫쨋날 취득
  $NC.setInitDatePicker("#dtpQInbound_Date1"); // 당월의 첫쨋날 취득
  $NC.setInitDatePicker("#dtpQInbound_Date2");
  $NC.setValue("#dtpQInbound_Date1", $NC.addDay($NC.getValue("#dtpQInbound_Date2"), -3));
...
function _Inquiry() {
  ...
  if ($("#divMasterView").tabs("option", "active") === 0) {
    ...
    // 데이터 조회
    $NC.serviceCall("/LI03040Q/getDataSet.do", $NC.getGridParams(G_GRDMASTERT1), onGetMasterT1);
    ...
    // 데이터 조회
    $NC.serviceCall("/LI03040Q/getDataSet.do", $NC.getGridParams(G_GRDMASTERT2), onGetMasterT2);
