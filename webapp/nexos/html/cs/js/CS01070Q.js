...
function _Inquiry() {
  ...
  // 데이터 조회
  $NC.serviceCall("/CS01070Q/getDataSet.do", {
    P_QUERY_ID: "CS01070Q.RS_MASTER",
...
function onGetMenu(ajaxData) {
  ...
  function viewFavorites() {
    favorite = parent.G_GRDPROGRAMBOOKMARK.data.getItems()
    for (var i in favorite) {
      var programId = favorite[i].PROGRAM_ID
      var item = $('[data-id="' + programId + '"]').addClass('favorite')
        .parent().addClass('favorite')
        .parent().addClass('favorite')
        .prev().addClass('favorite')
        .parent().addClass('favorite')
        .parent().addClass('favorite')
      if (favorite[i].indent == 2) {
        item.parent().addClass('favorite')
          .parent().addClass('favorite')
      }
    }
  }
