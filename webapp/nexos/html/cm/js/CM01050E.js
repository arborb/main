...
function _OnGridCheckBoxFormatterClick(e, view, args) {
  ...
  if (args.cell == G_GRDMASTER.view.getColumnIndex("NORMAL_YN")) {
    rowData.NORMAL_YN = args.val === "Y" ? "N" : "Y";
  } else if (args.cell == G_GRDMASTER.view.getColumnIndex("FROZEN_YN")) {
...
function _Inquiry() {
  ...
  // 데이터 조회
  $NC.serviceCall("/CM01050E/getDataSet.do", $NC.getGridParams(G_GRDMASTER), onGetMaster);
}
...
function _New() {
  ...
  if (rowCount > 0) {
    ...
    if (rowData.CRUD == "N") {
      G_GRDMASTER.view.gotoCell(rowCount - 1, G_GRDMASTER.view.getColumnIndex("DOCK_NO"), true);
      return;
    }
  }
  ...
  var newRowData = {
    ...
    MAX_TON_DIV_F: $NC.getGridComboName(G_GRDMASTER, {
      colFullNameField: "MAX_TON_DIV_F",
      searchVal: "999",
...
function _Save() {
  ...
  if (saveMasterDS.length > 0) {
    $NC.serviceCall("/CM01050E/save.do", {
...
function grdMasterOnNewRecord(args) {
  ...
  $NC.setFocusGrid(G_GRDMASTER, args.row, G_GRDMASTER.view.getColumnIndex("DOCK_NO"), true);
}

function grdMasterOnBeforeEditCell(e, args) {

  // 수정할 수 없는 컬럼일 경우 수정 모드로 변경하지 않도록 처리
  if (args.column.field !== "DOCK_NO") {
    return true;
  }
  var rowData = G_GRDMASTER.data.getItem(args.row);
  if (rowData) {
...
function grdMasterOnBeforePost(row) {
  ...
  if (rowData.CRUD != "R") {
    if ($NC.isNull(rowData.DOCK_NO)) {
      ...
      G_GRDMASTER.view.gotoCell(row, G_GRDMASTER.view.getColumnIndex("DOCK_NO"), true);
      return false;
    }
    if (rowData.NORMAL_YN == "N" && rowData.FROZEN_YN == "N") {
      alert("상온 또는 냉동/냉장 접안여부 중 하나는 선택해야 합니다.");
      $NC.setGridSelectRow(G_GRDMASTER, row);
      G_GRDMASTER.view.gotoCell(row, G_GRDMASTER.view.getColumnIndex("NORMAL_YN"), true);
      return false;
    }
    if ($NC.isNull(rowData.MAX_TON_DIV)) {
      alert("최대 접안톤구분을 선택하십시오.");
      $NC.setGridSelectRow(G_GRDMASTER, row);
      G_GRDMASTER.view.gotoCell(row, G_GRDMASTER.view.getColumnIndex("MAX_TON_DIV_F"), true);
      return false;
    }
  }
