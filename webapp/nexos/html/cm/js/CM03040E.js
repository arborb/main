...
function _Initialize() {
  ...
  $NC.setInitCombo("/WC/getDataSet.do", {
    ...
    fullNameField: "CODE_CD_F",
    onComplete: function() {
      $NC.setValue("#cboStock_First_Div", -1);
    }
  });
...
function _Inquiry() {
  ...
  // 데이터 조회
  $NC.serviceCall("/CM03040E/getDataSet.do", $NC.getGridParams(G_GRDMASTER), onGetMaster);
}
...
function _Save() {
  ...
  for (var row = 0; row < rowCount; row++) {
    ...
    if (rowData.CRUD !== "R") {
      var saveData = {
        ...
        P_DELIVERY_TIME: rowData.DELIVERY_TIME,
  ...
  if (saveDS.length > 0) {
    $NC.serviceCall("/CM03040E/save.do", {
...
function setInputValue(grdSelector, rowData) {
  ...
  if (grdSelector === "#grdMaster") {
    ...
    $NC.setValue("#edtDelivery_Time", rowData.DELIVERY_TIME);
