...
function _Inquiry() {
  ...
  var SAFETY_DATE = $NC.getValue("#dtpQSafety_Date");
  if ($NC.isNull(SAFETY_DATE)) {
    alert("안전재고계산일을 입력하십시오.");
    $NC.setFocus("#dtpQSafety_Date");
    return;
  }
  ...
  // 데이터 조회
  $NC.serviceCall("/CM06020E/getDataSet.do", $NC.getGridParams(G_GRDMASTER), onGetMaster);
}
...
function _Save() {
  ...
  if (saveMasterDS.length > 0) {
    $NC.serviceCall("/CM06020E/save.do", {
