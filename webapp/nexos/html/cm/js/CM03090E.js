...
function _Inquiry() {
  ...
  if ($("#divTabView").tabs("option", "active") === 0) {
    ...
    // 데이터 조회
    $NC.serviceCall("/CM03090E/getDataSet.do", $NC.getGridParams(G_GRDT1MASTER), onGetT1Master);
  } else {
    ...
    // 데이터 조회
    $NC.serviceCall("/CM03090E/getDataSet.do", $NC.getGridParams(G_GRDT2MASTER), onGetT2Master);
  }
...
function _Save() {
  ...
  if ($("#divTabView").tabs("option", "active") === 0) {
    ...
    if (saveDS.length > 0) {
      $NC.serviceCall("/CM03090E/save.do", {
...
function _Delete() {
  ...
  if ($("#divTabView").tabs("option", "active") === 0) {
    ...
    var result = confirm("위탁사의 판매사 매칭 정보를 삭제 하시겠습니까?");
    if (result) {
      $NC.serviceCall("/CM03090E/save.do", {
        P_DS_MASTER: $NC.toJson(saveDS),
        P_USER_ID: $NC.G_USERINFO.USER_ID
...
function grdT1MasterOnAfterScroll(e, args) {
  ...
  // 데이터 조회
  $NC.serviceCall("/CM03090E/getDataSet.do", $NC.getGridParams(G_GRDT1DETAIL), onGetT1Detail);
  ...
  // 데이터 조회
  $NC.serviceCall("/CM03090E/getDataSet.do", $NC.getGridParams(G_GRDT1SUB), onGetT1Sub);
...
function grdT2MasterOnAfterScroll(e, args) {
  ...
  // 데이터 조회
  $NC.serviceCall("/CM03090E/getDataSet.do", $NC.getGridParams(G_GRDT2DETAIL), onGetT2Detail);
  ...
  // 데이터 조회
  $NC.serviceCall("/CM03090E/getDataSet.do", $NC.getGridParams(G_GRDT2SUB), onGetT2Sub);
...
function onBtnAddBrand() {
  ...
  for (var row = 0; row < rowCount; row++) {
    ...
    if (rowData.CHECK_YN == "Y" && rowData.CHK == "N") {
      checkedValueDS.push(rowData.BRAND_CD);
    }
  }

  if (checkedValueDS.length == 0) {
    alert("이미 매칭된 판매사이거나 선택하신 판매사가 없습니다.");
    return;
  }

  var result = confirm("선택하신 화주를 대표위탁사코드에 추가 하시겠습니까?");
  if (result) {
    var rowData = G_GRDT1MASTER.data.getItem(G_GRDT1MASTER.lastRow);

    $NC.serviceCall("/CM03090E/callOwnBrandMatchBrand.do", {
      P_QUERY_PARAMS: $NC.getParams({
        P_BU_CD: rowData.BU_CD,
        P_OWN_BRAND_CD: rowData.OWN_BRAND_CD,
        P_USER_ID: $NC.G_USERINFO.USER_ID,
...
function onBtnAddVendor() {
  ...
  if (result) {
    ...
    $NC.serviceCall("/CM03090E/callVendorItemAddVendor.do", {
      P_QUERY_PARAMS: $NC.getParams({
        P_BRAND_CD: rowData.BRAND_CD,
        P_ITEM_CD: rowData.ITEM_CD,
        P_USER_ID: $NC.G_USERINFO.USER_ID,
