...
function _Inquiry() {
  ...
  // 데이터 조회
  $NC.serviceCall("/CM02050E/getDataSet.do", $NC.getGridParams(G_GRDMASTER), onGetMaster);
}
...
function _Save() {
  ...
  for (var row = 0; row < rowCount; row++) {
    ...
    if (rowData.CRUD !== "R") {
      var saveData = {
        ...
        P_CARRIER_FULL_NM: rowData.CARRIER_FULL_NM,
  ...
  if (saveDS.length > 0) {
    $NC.serviceCall("/CM02050E/save.do", {
...
function setInputValue(grdSelector, rowData) {
  ...
  if (grdSelector === "#grdMaster") {
    ...
    $NC.setValue("#edtCarrier_Full_Nm", rowData["CARRIER_FULL_NM"]);
...
function grdMasterOnBeforePost(row) {
  ...
  if (rowData.CRUD != "R") {
    ...
    if ($NC.isNull(rowData.CARRIER_NM)) {
      alert("운송사명을 입력하십시오.");
      $NC.setFocus("#edtCarrier_Nm");
      $NC.setGridSelectRow(G_GRDMASTER, row);
      return false;
    }
...
function grdMasterOnCellChange(e, args) {
  ...
  if (rowData) {
    ...
    switch (args.col) {
    case "CARRIER_CD":
      rowData.CARRIER_CD = args.val;
      break;
    case "CARRIER_NM":
      rowData.CARRIER_NM = args.val;
      if ($NC.isNull(rowData.CARRIER_FULL_NM)) {
        rowData.CARRIER_FULL_NM = rowData.CARRIER_NM;
        $NC.setValue("#edtCarrier_Full_Nm", rowData.CARRIER_FULL_NM);
      }
      break;
    case "CARRIER_FULL_NM":
      rowData.CARRIER_FULL_NM = args.val;
      break;
    case "BUSINESS_NO":
