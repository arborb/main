...
function _Inquiry() {
  ...
  if ($("#divTabView").tabs("option", "active") === 0) {
    ...
    // 데이터 조회
    $NC.serviceCall("/CM04060E/getDataSet.do", $NC.getGridParams(G_GRDT1MASTER), onGetT1Master);
  } else {
    ...
    // 데이터 조회
    $NC.serviceCall("/CM04060E/getDataSet.do", $NC.getGridParams(G_GRDT2MASTER), onGetT2Master);
  }
...
function _Save() {
  ...
  if ($("#divTabView").tabs("option", "active") === 0) {
    ...
    if (saveDS.length > 0) {
      $NC.serviceCall("/CM04060E/save.do", {
...
function grdT1MasterOnAfterScroll(e, args) {
  ...
  // 데이터 조회
  $NC.serviceCall("/CM04060E/getDataSet.do", $NC.getGridParams(G_GRDT1DETAIL), onGetT1Detail);
  ...
  // 데이터 조회
  $NC.serviceCall("/CM04060E/getDataSet.do", $NC.getGridParams(G_GRDT1SUB), onGetT1Sub);
...
function grdT1SubOnCellChange(e, args) {
  ...
  switch (G_GRDT1SUB.view.getColumnField(args.cell)) {
  case "OPEN_DATE":
    if (!$NC.isNull(rowData.OPEN_DATE)) {
      if (!$NC.isDate(rowData.OPEN_DATE)) {
        ...
        $NC.setGridSelectRow(G_GRDT1SUB, {
          selectRow: args.row,
          activeCell: G_GRDT1SUB.view.getColumnIndex("OPEN_DATE"),
          editMode: true
        });
        return false;
      } else {
  ...
  case "CLOSE_DATE":
    if (!$NC.isNull(rowData.CLOSE_DATE)) {
      if (!$NC.isDate(rowData.CLOSE_DATE)) {
        ...
        $NC.setGridSelectRow(G_GRDT1SUB, {
          selectRow: args.row,
          activeCell: G_GRDT1SUB.view.getColumnIndex("CLOSE_DATE"),
          editMode: true
        });
        return false;
      } else {
  ...
  // 마지막 선택 Row 수정 상태로 변경
  G_GRDT1SUB.lastRowModified = true;
}
...
function grdT2MasterOnAfterScroll(e, args) {
  ...
  // 데이터 조회
  $NC.serviceCall("/CM04060E/getDataSet.do", $NC.getGridParams(G_GRDT2DETAIL), onGetT2Detail);
  ...
  // 데이터 조회
  $NC.serviceCall("/CM04060E/getDataSet.do", $NC.getGridParams(G_GRDT2SUB), onGetT2Sub);
...
function onBtnAddItem() {
  ...
  for (var row = 0; row < rowCount; row++) {
    ...
    if (rowData.CHECK_YN == "Y") {
      checkedValueDS.push(rowData.BRAND_CD + ";" + rowData.ITEM_CD);
    }
  }
  ...
  var result = confirm("상품을 추가 하시겠습니까?");
  if (result) {
    var rowData = G_GRDT1MASTER.data.getItem(G_GRDT1MASTER.lastRow);

    $NC.serviceCall("/CM04060E/callVendorItemAddItem.do", {
      P_QUERY_PARAMS: $NC.getParams({
        P_CUST_CD: $NC.G_USERINFO.CUST_CD,
        P_VENDOR_CD: rowData.VENDOR_CD,
        P_USER_ID: $NC.G_USERINFO.USER_ID,
...
function onBtnAddVendor() {
  ...
  if (result) {
    ...
    $NC.serviceCall("/CM04060E/callVendorItemAddVendor.do", {
      P_QUERY_PARAMS: $NC.getParams({
        P_BRAND_CD: rowData.BRAND_CD,
        P_ITEM_CD: rowData.ITEM_CD,
        P_USER_ID: $NC.G_USERINFO.USER_ID,
