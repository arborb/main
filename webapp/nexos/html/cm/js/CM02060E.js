...
function _Inquiry() {
  ...
  // 데이터 조회
  $NC.serviceCall("/CM02060E/getDataSet.do", $NC.getGridParams(G_GRDMASTER), onGetMaster);
}
...
function _New() {
  ...
  if (rowCount > 0) {
    ...
    if (rowData.CRUD == "N") {
      $NC.setFocus("#edtMall_Cd");
      return;
    }
  }
  
  // 상품코드 받아 오기
  $NC.serviceCall("/CM02060E/getDataSet.do", {
    P_QUERY_ID: "CM02060E.GET_NEXTVAL",
    P_QUERY_PARAMS: ""
  }, function(ajaxData) {
    var resultData = $NC.toArray(ajaxData.data);
    var LAST_NUMBER = resultData[0].LAST_NUMBER;
    $NC.G_VAR.MALL_CD = LAST_NUMBER;
    $NC.setValue("#edtMall_Cd", LAST_NUMBER);
  });
...
function _Save() {
  ...
  for (var row = 0; row < rowCount; row++) {
    ...
    if (rowData.CRUD !== "R") {
      var saveData = {
        ...
        P_MALL_NM: rowData.MALL_NM,
        P_MALL_FULL_NM: rowData.MALL_FULL_NM,
  ...
  if (saveDS.length > 0) {
    $NC.serviceCall("/CM02060E/save.do", {
...
function _Delete() {
  ...
  if (result) {
    ...
    if (rowData.DELETE_CHK === "Y"){
      alert("해당 MALL은 딜과 연결되어있어 삭제가 불가능합니다.");
      return;
    }
...
function setInputValue(grdSelector, rowData) {
  ...
  if (grdSelector === "#grdMaster") {
    ...
    // Row 데이터로 에디터 세팅
    $NC.setValue("#edtMall_Cd", rowData["MALL_CD"]);
    $NC.setValue("#edtMall_Nm", rowData["MALL_NM"]);
    $NC.setValue("#edtMall_Full_Nm", rowData["MALL_FULL_NM"]);
...
//    $NC.setValue("#edtCharge_Hp", rowData["CHARGE_HP"]);
    ...
    if (rowData.CRUD == "C" || rowData.CRUD == "N") {
      $NC.setEnable("#edtMall_Cd");
    } else {
      $NC.setEnable("#edtMall_Cd", false);
    }
  }
}
...
function grdMasterOnBeforePost(row) {
  ...
  if (rowData.CRUD != "R") {
    if ($NC.isNull(rowData.MALL_CD)) {
      alert("MALL코드를 입력하십시오.");
      $NC.setFocus("#edtMall_Cd");
      $NC.setGridSelectRow(G_GRDMASTER, row);
      return false;
    }
    if ($NC.isNull(rowData.MALL_NM)) {
      alert("MALL명을 입력하십시오.");
      $NC.setFocus("#edtMall_Nm");
      $NC.setGridSelectRow(G_GRDMASTER, row);
      return false;
    }
...
function grdMasterOnCellChange(e, args) {
  ...
  if (rowData) {
    ...
    switch (args.col) {
    case "MALL_CD":
      rowData.MALL_CD = args.val;
      break;
    case "MALL_NM":
      rowData.MALL_NM = args.val;
      if ($NC.isNull(rowData.MALL_FULL_NM)) {
        rowData.MALL_FULL_NM = rowData.MALL_NM;
        rowData.MALL_CD = $NC.G_VAR.MALL_CD;
        $NC.setValue("#edtMall_Full_Nm", rowData.MALL_FULL_NM);
      }
      break;
    case "MALL_FULL_NM":
      rowData.MALL_FULL_NM = args.val;
      break;
    case "MALL_DIV":
      rowData.MALL_DIV = args.val;
      break;
    case "BUSINESS_NO":
