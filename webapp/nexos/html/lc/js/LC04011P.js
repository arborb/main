...
function _OnPopupOpen() {
  ...
  if ($NC.G_VAR.userData.P_PROCESS_CD === "N") {
    ...
    $NC.setValue("#cboInvest_Div", masterDS.INVEST_DIV);

    $NC.setValue("#edtInvest_Div_Org", masterDS.INVEST_DIV);
    // $NC.setValue("#cboInvest_Type",masterDS.INVEST_TYPE);
    ...
    $NC.G_VAR.masterData = {
      ...
      INVEST_TYPE: masterDS.INVEST_TYPE,
      INVEST_DIV: masterDS.INVEST_DIV,
      INVEST_DIV_F: masterDS.INVEST_DIV_F,
      INVEST_START_DATE: masterDS.INVEST_START_DATE,
      INVEST_END_DATE: masterDS.INVEST_END_DATE,
...
function _New() {
  ...
  case "1":
    ...
    $NC.serviceCall("/LC04010E/getDataSet.do", {
      // P_QUERY_ID: "LC04010E.RS_POP_SUB1",
      P_QUERY_ID: "LC04010E.RS_POP_SUB7",
      P_QUERY_PARAMS: $NC.getParams({
  ...
  case "2":
    ...
    // 로케이션 조회
    $NC.serviceCall("/LC04010E/getDataSet.do", {
      P_QUERY_ID: "LC04010E.RS_POP_SUB2",
      P_QUERY_PARAMS: $NC.getParams({
        P_CENTER_CD: CENTER_CD,
        P_BU_CD: BU_CD,
        P_BRAND_CD: QBRAND_CD,
  ...
  case "3":
    ...
    // 고정로케이션 조회
    $NC.serviceCall("/LC04010E/getDataSet.do", {
      P_QUERY_ID: "LC04010E.RS_POP_SUB3",
      P_QUERY_PARAMS: $NC.getParams({
        P_CENTER_CD: CENTER_CD,
        P_BRAND_CD: QBRAND_CD,
        P_BU_CD: BU_CD,
  ...
  case "4":
    ...
    if (INOUT_DATE1 > INOUT_DATE2) {
      alert("기간 검색 범위 오류입니다.");
      $NC.setFocus("#dtpQInout_Date1");
      return;
    }

    var ITEM_STATE = $NC.getValue("#cboQItem_State");
    var INOUT_GRP = $NC.getValueRadioGroup("#DEAL_DIV") + "%";

    // 기간입출고 조회
    $NC.serviceCall("/LC04010E/getDataSet.do", {
      P_QUERY_ID: "LC04010E.RS_POP_SUB4",
      P_QUERY_PARAMS: $NC.getParams({
        P_CENTER_CD: CENTER_CD,
        P_BU_CD: BU_CD,
        P_BRAND_CD: QBRAND_CD,
  ...
  case "5":
    ...
    if ($NC.isNull(KBRAND_CD)) {
      alert("브랜드를 선택하여 주십시오.");
      $NC.setFocus("#edtKBrand_Cd");
      return;
    }

    if ($NC.isNull(DEPART_CD) && $NC.isNull(LINE_CD) && $NC.isNull(CLASS_CD)) {
      alert("검색조건중 대,중,소분류를 정확히 선택하여 주십시오");
      return;
    }

    // 상품그룹 조회
    $NC.serviceCall("/LC04010E/getDataSet.do", {
      P_QUERY_ID: "LC04010E.RS_POP_SUB5",
      P_QUERY_PARAMS: $NC.getParams({
        P_CENTER_CD: CENTER_CD,
        P_BU_CD: BU_CD,
        P_BRAND_CD: KBRAND_CD,
        P_ITEM_STATE: ITEM_STATE,
  ...
  case "6":
    ...
    if (ITEM_CD === "%" && $NC.isNull(ZONE_CD) && $NC.isNull(BANK_CD) && $NC.isNull(BAY_CD) && $NC.isNull(LEV_CD)) {
      alert("검색조건 중 하나는 반드시 입력 하셔야 합니다.");
      return;
    }
    ...
    $NC.serviceCall("/LC04010E/getDataSet.do", {
      P_QUERY_ID: "LC04010E.RS_POP_SUB6",
      P_QUERY_PARAMS: $NC.getParams({
        P_CENTER_CD: CENTER_CD,
        P_BU_CD: BU_CD,
        P_BRAND_CD: QBRAND_CD,
...
function _Save() {
  ...
  if ($NC.G_VAR.masterData.INVEST_DIV !== '1') {
    var detailDS = [ ];
    ...
    var INVEST_DIV = $NC.G_VAR.masterData.INVEST_DIV;

    if ($NC.G_VAR.userData.P_PROCESS_CD === "N") {
      INVEST_DIV = $NC.G_VAR.masterData.INVEST_DIV;
    } else {
      INVEST_DIV = $NC.getValue("#edtInvest_Div_Org");
    }

    $NC.serviceCall("/LC04010E/callLcsave.do", {
      P_DS_MASTER: $NC.toJson({
        P_CENTER_CD: $NC.G_VAR.masterData.CENTER_CD,
        P_BU_CD: $NC.G_VAR.masterData.BU_CD,
        P_INVEST_DATE: $NC.G_VAR.masterData.INVEST_DATE,
        P_INVEST_NO: $NC.G_VAR.masterData.INVEST_NO,
        // P_INVEST_TYPE:$NC.G_VAR.masterData.INVEST_TYPE,
        P_INVEST_DIV: INVEST_DIV,
    ...
    if ($NC.G_VAR.masterData.CRUD === "C") {
      $NC.serviceCall("/LC04010E/callSP.do", {
        P_QUERY_ID: "LC_040_ALLSAVE",
        P_QUERY_PARAMS: $NC.getParams({
          P_CENTER_CD: $NC.G_VAR.masterData.CENTER_CD,
          P_BU_CD: $NC.G_VAR.masterData.BU_CD,
          P_BRAND_CD: '%',
...
function masterDataOnChange(e, args) {
  ...
  case "INVEST_TYPE":
    $NC.G_VAR.masterData.INVEST_TYPE = args.val;
    break;
...
function _OnConditionChange(e, view, val) {
  ...
  case "QBRAND_CD":
    ...
    if (O_RESULT_DATA.length <= 1) {
      onQBuBrandPopup(O_RESULT_DATA[0]);
    } else {
      $NP.showOwnBranPopup({
        queryParams: P_QUERY_PARAMS,
        queryData: O_RESULT_DATA
      }, onQBuBrandPopup, onQBuBrandPopup);
    }
    break;
  case "KBRAND_CD":
    var P_QUERY_PARAMS;
    ...
    if (O_RESULT_DATA.length <= 1) {
      onKBuBrandPopup(O_RESULT_DATA[0]);
    } else {
      $NP.showOwnBranPopup({
        queryParams: P_QUERY_PARAMS,
        queryData: O_RESULT_DATA
      }, onKBuBrandPopup, onKBuBrandPopup);
    }
    break;
  case "KITEM_CD":
    var P_QUERY_PARAMS;
...
function setPolicyValInfo() {
  ...
  for ( var POLICY_CD in $NC.G_VAR.policyVal) {
    // 데이터 조회
    $NC.serviceCallAndWait("/LC04010E/callSP.do", {
...
function onKBuBrandPopup(resultInfo) {
  ...
  if (!$NC.isNull(resultInfo)) {
    $NC.setValue("#edtKBrand_Cd", resultInfo.OWN_BRAND_CD);
    $NC.setValue("#edtKBrand_Nm", resultInfo.OWN_BRAND_NM);
