...
function _Initialize() {
  ...
  $("#edtBu_Cd").prop("readonly", true); // readonly
...
function _Inquiry() {
  ...
  */
  ...
  // 데이터 조회
  $NC.serviceCall("/LC02020E/getDataSet.do", {
    P_QUERY_ID: "LC02020E.RS_POP_SUB1",
    P_QUERY_PARAMS: $NC.getParams({
      P_CENTER_CD: $NC.G_VAR.userData.P_CENTER_CD,
...
function _New() {
  ...
  for ( var row in detailDs) {
    ...
    var newRowData = {
      ...
      LINK_ITEM_CD: "",
      LINK_ITEM_NM: "",
      LINK_ITEM_STATE: rowData.ITEM_STATE,
      LINK_ITEM_SPEC: "",
      LINK_ITEM_LOT: "00",
      LINK_QTY_IN_BOX: 1,
...
function _Save() {
  ...
  $NC.serviceCall("/LC02020E/save.do", {
...
function chkSubParameter(row, rowData) {
  ...
  if (rowData.CRUD == "C" && $NC.isNull(rowData.LINK_ITEM_CD)) {
    alert("변환상품을 입력하십시오.");
    $NC.setGridSelectRow(G_GRDSUB, {
      selectRow: row,
      activeCell: G_GRDSUB.view.getColumnIndex("LINK_ITEM_CD"),
      editMode: true
    });
    return false;
  }
  if (rowData.CRUD == "C" && $NC.isNull(rowData.LINK_ITEM_LOT)) {
    alert("변환상품의 LOT번호를 입력하십시오.");
    $NC.setGridSelectRow(G_GRDSUB, {
      selectRow: row,
      activeCell: G_GRDSUB.view.getColumnIndex("LINK_ITEM_LOT"),
      editMode: true
    });
    return false;
  }
...
function setPolicyValInfo() {
  ...
  for ( var POLICY_CD in $NC.G_VAR.policyVal) {
    // 데이터 조회
    $NC.serviceCallAndWait("/LC02020E/callSP.do", {
...
function onGridLocationPopup(resultInfo) {
  ...
  if (!$NC.isNull(resultInfo)) {
    ...
    if (rowData) {
      ...
      $NC.setFocusGrid(G_GRDSUB, G_GRDSUB.lastRow, G_GRDSUB.view.getColumnIndex("LINK_ITEM_CD"), true, true);
    }
  } else {
    var rowData = G_GRDSUB.data.getItem(G_GRDSUB.lastRow);
    if (rowData) {
...
function onGridItemPopup(resultInfo) {
  ...
  if (!$NC.isNull(resultInfo)) {
    ...
    if (rowData) {
      ...
      // 수정 상태로 변경
      G_GRDSUB.lastRowModified = true;
      $NC.setFocusGrid(G_GRDSUB, G_GRDSUB.lastRow, G_GRDSUB.view.getColumnIndex("CONFIRM_QTY"), true, true);
