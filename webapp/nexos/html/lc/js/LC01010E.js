...
function _Inquiry() {
  ...
  var etc_div = $(':radio[name="rgbQInout_Div"]:checked').val();
  var INOUT_CD = $NC.getValue("#cboQInout_Cd" + etc_div);
  if ($NC.isNull(INOUT_CD)) {
    alert("입고구분을 선택하십시오.");
    $NC.setFocus("#cboQInout_Cd" + etc_div);
    return;
  }
  if (etc_div != "" && INOUT_CD == "%") INOUT_CD = etc_div + INOUT_CD;
  ...
  if ($("#divMasterView").tabs("option", "active") === 0) {
    ...
    // 데이터 조회
    $NC.serviceCall("/LC01010E/getDataSet.do", $NC.getGridParams(G_GRDT1MASTER), onGetMasterT1);
    ...
    // 데이터 조회
    $NC.serviceCall("/LC01010E/getDataSet.do", $NC.getGridParams(G_GRDT2MASTER), onGetMasterT2);
  }
}
...
function _Delete() {
  ...
  if ($("#divMasterView").tabs("option", "active") === 0) {
    ...
    if (result) {
      $NC.serviceCall("/LC01010E/callLCProcessing.do", {
        P_PROCESS_DIV: "DEL",
        P_QUERY_PARAMS: $NC.getParams({
...
function _Print(printIndex, printName) {
  ...
  if (printIndex === 2) {
    ...
    var printOptions = {
      reportDoc: "lc/PAPER_LC01",
      queryId: "WR.RS_PAPER_LC01",
      queryParams: {
        P_CENTER_CD: rowData.CENTER_CD,
        P_BU_CD: rowData.BU_CD,
        P_ETC_DATE: rowData.ETC_DATE,
    ...
    if (rowData.INOUT_SUB_CD.substr(0, 1) !== "E") {
      alert("기타출고 전표입니다. \n기타입고 데이터를 선택하시오.");
      return;
    }
    var printOptions = {
      ...
      queryParams: {
        P_CENTER_CD: rowData.CENTER_CD,
        P_BU_CD: rowData.BU_CD,
        P_INBOUND_DATE: rowData.ETC_DATE,
        P_INBOUND_NO: rowData.ETC_NO,
        P_PROCESS_GRP: "LC"
      }
    };
...
function tabOnActivate(event, ui) {
  ...
  if (id === "TAB1") {
    ...
    if ($NC.isSplitter("#divDetailViewT1")) {
      ...
      $("#divDetailViewT1").trigger("resize");
    } else {
      // 스플리터 초기화
...
function grdT1MasterOnDblClick(e, args) {
  ...
  $NC.serviceCall("/LC01010E/getConfirmYn.do", {
    P_QUERY_PARAMS: $NC.getParams({
    ...
    if (masterRowData) {
      ...
      // 기타입출고 지시 데이터 데이터 조회 - 하단그리드
      $NC.serviceCall("/LC01010E/getDataSet.do", {
        P_QUERY_ID: "LC01010E.RS_POP_SUB2",
        P_QUERY_PARAMS: $NC.getParams({
          P_CENTER_CD: masterRowData.CENTER_CD,
          P_BU_CD: masterRowData.BU_CD,
          P_ETC_DATE: masterRowData.ETC_DATE,
...
function grdT1MasterOnAfterScroll(e, args) {
  ...
  // 데이터 조회
  $NC.serviceCall("/LC01010E/getDataSet.do", $NC.getGridParams(G_GRDT1DETAIL), onGetDetailT1);
  ...
  if (G_GRDT2MASTER.data.getLength() > 0) {
    if ($NC.isNull(G_GRDT2MASTER.lastKeyVal)) {
      ...
      $NC.setGridSelectRow(G_GRDT2MASTER, {
        selectKey: new Array("ETC_DATE", "ETC_NO", "LINE_NO"),
