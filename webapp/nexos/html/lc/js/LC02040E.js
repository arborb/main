...
function _Inquiry() {
  ...
  if ($("#divMasterView").tabs("option", "active") === 0) {
    ...
    // 데이터 조회
    $NC.serviceCall("/LC02040E/getDataSet.do", $NC.getGridParams(G_GRDT1MASTER), onGetT1Master);
    ...
    // 데이터 조회
    $NC.serviceCall("/LC02040E/getDataSet.do", $NC.getGridParams(G_GRDT2MASTER), onGetT2Master);
  }
}
...
function _Delete() {
  ...
  if ($("#divMasterView").tabs("option", "active") === 0) {
    ...
    if (result) {
      $NC.serviceCall("/LC02040E/callLCProcessing.do", {
        P_PROCESS_DIV: "DEL",
        P_QUERY_PARAMS: $NC.getParams({
...
function _Print(printIndex, printName) {
  ...
  if (printIndex === 2) {
    ...
    var printOptions = {
      reportDoc: "lc/PAPER_CHANGE02",
      queryId: "WR.RS_PAPER_CHANGE02",
      queryParams: {
        P_CENTER_CD: rowData.CENTER_CD,
        P_BU_CD: rowData.BU_CD,
        P_ETC_DATE: rowData.ETC_DATE,
...
function grdT1MasterOnAfterScroll(e, args) {
  ...
  // 데이터 조회
  $NC.serviceCall("/LC02040E/getDataSet.do", $NC.getGridParams(G_GRDT1DETAIL), onGetT1Detail);
...
function onSave(ajaxData) {
  ...
  _Inquiry();
  if (lastRowData) {
    G_GRDT1MASTER.lastKeyVal = new Array(lastRowData.ETC_DATE, lastRowData.ETC_NO);
  }
}
