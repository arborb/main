...
function _Initialize() {
  ...
  // 사업부이관 확정/취소 버튼 권한 체크 및 클릭 이벤트 연결
...
function _Inquiry() {
  ...
  if ($("#divMasterView").tabs("option", "active") === 0) {
    ...
    // Master 데이터 조회
    $NC.serviceCall("/LC02030E/getDataSet.do", $NC.getGridParams(G_GRDT1MASTER), onGetMasterT1);
  } else {
    ...
    // Master 데이터 조회
    $NC.serviceCall("/LC02030E/getDataSet.do", $NC.getGridParams(G_GRDT2MASTER), onGetMasterT2);
  }
}
...
function _Delete() {
  ...
  if ($("#divMasterView").tabs("option", "active") === 0) {
    ...
    if (result) {
      $NC.serviceCall("/LC02030E/callLCProcessing.do", {
        P_PROCESS_DIV: "DEL",
        P_QUERY_PARAMS: $NC.getParams({
...
function _Print(printIndex, printName) {
  ...
  if (printIndex === 2) {
    ...
    var printOptions = {
      reportDoc: "lc/PAPER_CHANGE03",
      queryId: "WR.RS_PAPER_CHANGE03",
      queryParams: {
        P_CENTER_CD: rowData.CENTER_CD,
        P_BU_CD: rowData.BU_CD,
        P_ETC_DATE: rowData.ETC_DATE,
...
function grdT1MasterOnDblClick(e, args) {
  ...
  $NC.serviceCall("/LC03020E/getConfirmYn.do", {
    ...
    if (rowData) {
      ...
      var CENTER_CD_F = $("#cboQCenter_Cd option:selected").text();
      var BU_CD_OUT = rowData.BU_CD;
      var BU_NM_OUT = rowData.BU_NM;
      var BU_CD_IN = rowData.LINK_BU_CD;
      var BU_NM_IN = rowData.LINK_BU_NM;
      var LINK_ETC_NO = rowData.LINK_ETC_NO;

      // 데이터 조회 - 하단그리드
      $NC.serviceCall("/LC02030E/getDataSet.do", {
        P_QUERY_ID: "LC02030E.RS_POP_SUB2",
        P_QUERY_PARAMS: $NC.getParams({
          P_CENTER_CD: rowData.CENTER_CD,
          P_BU_CD: rowData.BU_CD,
          P_ETC_DATE: rowData.ETC_DATE,
        ...
        $NC.G_MAIN.showProgramSubPopup({
          ...
          userData: {
            ...
            P_CENTER_CD: CENTER_CD,
            P_CENTER_CD_F: CENTER_CD_F,
            P_BU_CD_OUT: BU_CD_OUT,
            P_BU_NM_OUT: BU_NM_OUT,
            P_BU_CD_IN: BU_CD_IN,
            P_BU_NM_IN: BU_NM_IN,
            P_ETC_DATE: rowData.ETC_DATE,
            P_ETC_NO: rowData.ETC_NO,
            P_LINK_ETC_NO: LINK_ETC_NO,
            P_CUST_CD: $NC.getValue("#edtQCust_Cd"),
            P_BU_CD: BU_CD_IN,
            P_MASTER_DS: rowData,
          ...
          onOk: function() {
            G_GRDT1MASTER.lastKeyVal = new Array(rowData.ETC_DATE, rowData.ETC_NO);
            onSave();
          }
        });
      });
...
function grdT1MasterOnAfterScroll(e, args) {
  ...
  if (G_GRDT1MASTER.lastRow !== null) {
    ...
    // Master 데이터 조회
    $NC.serviceCall("/LC02030E/getDataSet.do", $NC.getGridParams(G_GRDT1DETAIL), onGetDetailT1);
  }
}
...
function onProcessPre() {
  ...
  if (G_GRDT1MASTER.lastRow == null) {
    alert("사업부이관등록 취소 처리할 데이터를 선택하십시오.");
    return;
  }
