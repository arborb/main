...
(function($) {
    ...
    $.jqplot.MeterGaugeRenderer.prototype.draw = function (ctx, gd, options) {
        ...
        var pd = this.padding; 
        if (pd == null) {
            pd = Math.round(Math.min(cw, ch)/30);
        }
        var w = cw - offx - 2 * pd;
        var h = ch - offy - 2 * pd;
        ...
        //if (!this.diameter) {
            if (this.semiCircular) {
                if ( w >= 2*h) {
                    ...
                    this.needleThickness = this.needleThickness || 2+Math.pow(rw, 0.8);
                    this.innerPad = rw/2 + this.needleThickness/2 + this.needlePad;
                    ...
                    this.needleThickness = this.needleThickness || 2+Math.pow(rw, 0.8);
                    this.innerPad = rw/2 + this.needleThickness/2 + this.needlePad;
                    this.diameter = w - 2*this.innerPad - rw - pd;
                }
                // center taking into account legend and over draw for gauge bottom below hub.
                // this will be center of hub.
                this._center = [(cw - trans * offx)/2 + trans * offx,  (ch + trans*offy - pd - rw - this.innerPad)];
            }
            else {
                if (!rw) {
                    rw = d/35;
        ...
        this._radius = this.diameter * 0.65;
        
        var ts = 6000/this.diameter;
        
        //if (!this.hubRadius) {
            this.hubRadius = this.diameter/20;
        //}
        
        this.shadowOffset = 0.5 + rw/9;
        this.shadowWidth = rw*1;
        
        this.tickPadding = 3 + Math.pow(this.diameter/20, 0.7);
        this.tickOuterRadius = this._radius - rw/2 - this.tickPadding;
        ...
        if (this.semiCircular) {
            ...
            if (this.label && this.labelPosition == 'inside') {
                ...
                var t = this._center[1] + this.canvas._offsets.top + this.innerPad + rw + pd + this.labelHeightAdjust;
                this._labelElem.css({left:l, top:t});
            ...
            if (this.intervalOuterRadius == null) {
                if (this.showTickLabels) {
                    outer = (this.tickOuterRadius - this.tickLength - this.tickPadding - this.diameter/8);
                }
                else {
                    outer = (this.tickOuterRadius - this.tickLength - this.diameter/16);
                }
            }
            else {
                outer = this.intervalOuterRadius;
            }
            var range = this.max - this.min;
            ...
            if (this.data.length > 0) {
                datapoint = this.data[0][1];
            }
            var dataspan = this.max - this.min;
            if (this.pegNeedle) {
                if (datapoint > this.max + dataspan*3/this.span) {
                    datapoint = this.max + dataspan*3/this.span;
                }
                if (datapoint < this.min - dataspan*3/this.span) {
            ...
            ctx.save();
            ctx.beginPath();  
            ctx.fillStyle = this.needleRingColor;
            ctx.arc(0 ,0, this.hubRadius, hubsa, hubea, false);
            ctx.closePath();
            ctx.fill();
            ctx.restore();
